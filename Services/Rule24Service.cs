        ...
        public async Task<Rule24WorkspaceSaveResult> SaveWorkspaceAsync(Rule24ValidationRequest request, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                ...
                if (existingSummary != null)
                {
                    ...
                    existingSummary.QualTable = request.QualTable;
                    existingSummary.AuditTable = request.AuditTable;
                    existingSummary.H16Table = request.H16Table;
                    existingSummary.QualCodeColumn = request.QualCodeColumn;
                    existingSummary.ApprovalStatusColumn = request.ApprovalStatusColumn;
                    existingSummary.ExcludedApprovalStatusValue = request.ExcludedApprovalStatusValue;
                    existingSummary.Control1OnlyMode = request.Control1OnlyMode;
                    existingSummary.AuditQualCodeColumn = request.AuditQualCodeColumn;
                    existingSummary.H16QualCodeColumn = request.H16QualCodeColumn;
...
ORDER BY RunTimestamp DESC, RunID DESC;";
        ...
        private static Rule24ValidationSummary? DeserializeSummary(string? json)
        {
            ...
            try
            {
                return JsonConvert.DeserializeObject<Rule24ValidationSummary>(json);
            }
            catch
            {
                return null;
