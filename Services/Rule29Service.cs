...
{
    public class Rule29Service : IRule29Service
    {
        private readonly IConfiguration _configuration;

        public Rule29Service(IConfiguration configuration)
        {
        ...
        public async Task<Rule29ColumnSelectionResult> GetColumnsAsync(string server, string database, string driver, string tableName)
        {
            try
            {
                ...
                var autoFilterColumn = FindFirst(columns,
                    ...
                    ["error", "code"]);
                var autoBreakdownColumn = FindFirst(columns,
                    ["Error_Type", "Erro_Type", "ErrorType"],
                    ["error_type", "errortype", "type"]);

                return new Rule29ColumnSelectionResult
                {
                    Success = true,
                    Columns = columns,
        ...
        public async Task<Rule29FilterValueResult> GetFilterValuesAsync(string server, string database, string driver, string tableName, string filterColumn)
        {
            try
            {
...
ORDER BY COUNT(*) DESC, FilterValue ASC;";
                ...
                return new Rule29FilterValueResult
                {
                    Success = true,
                    Options = options,
        ...
        public async Task<Rule29VerifyResult> VerifyTableAsync(Rule29VerifyRequest request)
        {
            try
            {
...
WHERE {countPredicate};";
                ...
                await using var sampleCommand = conn.CreateCommand();
...
ORDER BY NEWID();";
                ...
                var sampleRows = new List<Rule29SampleRowViewModel>();
                while (await reader.ReadAsync())
                {
                    var row = new Rule29SampleRowViewModel();
        ...
        public async Task<Rule29ValidationSummary> RunValidationAsync(Rule29ValidationRequest request, string? userEmail = null, string? userName = null)
        {
            try
            {
                ValidateRequest(request);
            ...
            {
                return new Rule29ValidationSummary
                {
                    Success = false,
                    Error = ex.Message
                };
        ...
        public async Task<Rule29WorkspaceStateViewModel?> GetCurrentWorkspaceStateAsync(int clientId, string? currentUserEmail = null, bool includeSummary = true)
        {
            await using var connection = await OpenSystemConnectionAsync();
...
ORDER BY vr.RunTimestamp DESC, vr.RunID DESC;";
            ...
            var workspace = new Rule29WorkspaceStateViewModel
            {
                ...
                FilterValue = reader.IsDBNull(7) ? "00708" : reader.GetString(7),
                ...
                SampleSize = deserializedSummary?.SampleSize ?? 25,
                ShowAllRecords = deserializedSummary?.ShowAllRecords ?? false,
                Summary = summary
            };
        ...
        public async Task<Rule29RunReviewViewModel?> GetSavedRunAsync(int runId, string? currentUserEmail = null)
        {
            await using var connection = await OpenSystemConnectionAsync();
            var currentUserId = await GetSystemUserIdByEmailAsync(connection, currentUserEmail);
...
WHERE vr.RunID = @RunID
  AND vr.RuleNumber = 29;";
            ...
            var viewModel = new Rule29RunReviewViewModel
            {
                RunId = reader.GetInt32(0),
                ClientId = reader.GetInt32(1),
        ...
        public async Task<Rule29WorkspaceSaveResult> SaveWorkspaceAsync(Rule29ValidationRequest request, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                if (request.RunId is null || request.RunId <= 0)
                {
                    return new Rule29WorkspaceSaveResult
                    {
                        Success = false,
                ...
                if (existingSummary != null)
                {
                    ...
                    existingSummary.FilterColumn = request.FilterColumn;
                    existingSummary.FilterValue = request.FilterValue;
                    existingSummary.BreakdownColumn = request.BreakdownColumn;
                    existingSummary.SampleSize = request.SampleSize;
                    existingSummary.ShowAllRecords = request.ShowAllRecords;
...
WHERE RunID = @RunID
  AND ClientID = @ClientID;";
        ...
        public async Task<Rule29WorkspaceSaveResult> BeginWorkspaceEditAsync(int runId, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                await using var connection = await OpenSystemConnectionAsync();
...
WHERE RunID = @RunID;";
                ...
                markEdit.Parameters.AddWithValue("@RecordHash", ComputeHash($@"BeginWorkspaceEdit|Rule29|{runId}|{reviewerEmail}|{DateTime.UtcNow:o}|{previousHash}"));
...
WHERE RunID = @RunID
  ...
  AND SignoffRole = @SignoffRole;";
        ...
        public Task<string> GenerateSqlAsync(Rule29ValidationRequest request)
        {
            ...
            var showAll = request.ShowAllRecords;
            var sampleSize = Math.Max(request.SampleSize, 1);
            var hasBreakdown = !string.IsNullOrWhiteSpace(request.BreakdownColumn);
            var safeBreakdownColumn = hasBreakdown ? Sanitise(request.BreakdownColumn) : "";

            var sql = $@"-- ============================================================================
-- HEMIS RULE 29: SINGLE COLUMN FILTER
-- ============================================================================
-- Table: [{safeTable}]
...
-- Breakdown Column: {(hasBreakdown ? $"[{safeBreakdownColumn}]" : "(not selected)")}
-- ============================================================================";
...
";
            ...
            {
                sql += $@"
-- STEP 3: BREAKDOWN BY [{safeBreakdownColumn}]
SELECT
    LTRIM(RTRIM(CAST([{safeBreakdownColumn}] AS nvarchar(255)))) AS Breakdown_Value,
    COUNT(*) AS Record_Count,
    CAST(COUNT(*) * 100.0 / NULLIF(SUM(COUNT(*)) OVER (), 0) AS decimal(10,2)) AS Percentage
FROM [{safeTable}]
WHERE {sqlFilterPredicate}
GROUP BY LTRIM(RTRIM(CAST([{safeBreakdownColumn}] AS nvarchar(255))))
ORDER BY COUNT(*) DESC, Breakdown_Value ASC;
";
            }

            sql += $@"
-- STEP 4: GET MATCHING RECORDS
SELECT {(showAll ? "" : $"TOP {sampleSize} ")}*
FROM [{safeTable}]
WHERE {sqlFilterPredicate}
{(showAll ? ";" : "ORDER BY NEWID();")}
...
WHERE {countPredicate};";
            ...
            var breakdown = new List<Rule29BreakdownItemViewModel>();
            if (!string.IsNullOrWhiteSpace(safeBreakdownColumn))
            {
                await using var breakdownCommand = conn.CreateCommand();
                var breakdownPredicate = BuildFilterPredicate(breakdownCommand, safeFilterColumn, parsedValues, normalizedValues);
                breakdownCommand.CommandText = $@"
SELECT
    LTRIM(RTRIM(CAST([{safeBreakdownColumn}] AS nvarchar(255)))) AS BreakdownValue,
    COUNT(*) AS RecordCount
FROM [{safeTable}]
...
ORDER BY COUNT(*) DESC, BreakdownValue ASC;";
                ...
                {
                    breakdown.Add(new Rule29BreakdownItemViewModel
                    {
            ...
            var matchingRows = new List<Rule29ValidationRowRecord>();
            if (matchingCount > 0)
            {
...
SELECT {(request.ShowAllRecords || matchingCount <= sampleSize ? "" : $"TOP {sampleSize} ")}*
FROM [{safeTable}]
WHERE {dataPredicate}
{(!(request.ShowAllRecords || matchingCount <= sampleSize) ? "ORDER BY NEWID()" : "")};";
                ...
                while (await reader.ReadAsync())
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(request.BreakdownColumn))
                        displayValues.TryGetValue(request.BreakdownColumn, out breakdownValue);

                    matchingRows.Add(new Rule29ValidationRowRecord
                    {
            ...
            var sampled = !request.ShowAllRecords && matchingCount > displayedCount;
            ...
            return new Rule29ValidationSummary
            {
                ...
                BreakdownColumn = request.BreakdownColumn,
                SampleSize = sampleSize,
                ShowAllRecords = request.ShowAllRecords,
                Sampled = sampled,
        ...
        private async Task<int> SaveValidationRunAsync(Rule29ValidationRequest request, Rule29ValidationSummary summary, string? userEmail, string? userName)
        {
            await using var connection = await OpenSystemConnectionAsync();
            await EnsureClientNotArchivedAsync(connection, request.ClientId);
            await MarkPreviousRunsHistoricalAsync(connection, request.ClientId, 29);
            ...
            var previousHash = await GetLatestValidationHashAsync(connection, request.ClientId, 29);
...
VALUES
(
    @ClientID, @UserID, 29, @RuleName, @Status, @TotalRecords, @PassCount, @FailCount, @ExceptionRate, GETDATE(),
...
SELECT CAST(SCOPE_IDENTITY() AS int);";
            ...
            command.Parameters.AddWithValue("@RuleName", "Single Column Filter");
            ...
            command.Parameters.AddWithValue("@ExceptionsJSON", JsonConvert.SerializeObject(summary.MatchingRows));
...
WHERE RunID = @RunID;";
            ...
            hashCommand.Parameters.AddWithValue("@RecordHash", ComputeHash($@"ValidationRun|Rule29|{runId}|{request.ClientId}|{systemUserId.Value}|{summary.Status}|{summary.TotalValidated}|{summary.FailCount}|{summary.ExceptionRate}|{summary.Timestamp}|{previousHash}"));
...
ORDER BY ORDINAL_POSITION;";
        ...
        private static void ValidateRequest(Rule29ValidationRequest request)
        {
        ...
        private static void ValidateSqlRequest(Rule29ValidationRequest request)
        {
            if (string.IsNullOrWhiteSpace(request.Database))
                throw new InvalidOperationException("Database is required.");
            if (string.IsNullOrWhiteSpace(request.TableName))
        ...
        private static void ValidateRequest(Rule29VerifyRequest request)
        {
            if (string.IsNullOrWhiteSpace(request.Server))
                throw new InvalidOperationException("Server name is required.");
            if (string.IsNullOrWhiteSpace(request.Database))
...
ORDER BY RunTimestamp DESC, RunID DESC;";
        ...
        private async Task<Rule29ValidationSummary?> GetValidationSummaryAsync(SqlConnection connection, int runId)
        {
            await using var command = connection.CreateCommand();
        ...
        private static Rule29ValidationSummary? DeserializeSummary(string? json)
        {
            ...
            try
            {
                return JsonConvert.DeserializeObject<Rule29ValidationSummary>(json);
            }
            catch
            {
                return null;
