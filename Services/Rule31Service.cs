...
{
    public class Rule31Service : IRule31Service
    {
        ...
        public Rule31Service(IConfiguration configuration, IPendingValidationCacheService pendingValidationCache)
        {
        ...
        public async Task<TableListResult> GetTablesAsync(string server, string database, string driver)
        {
            try
            {
                ...
                var autoTable = tables.FirstOrDefault(t =>
                        t.Equals("dbo_QUAL_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase))
                    ?? tables.FirstOrDefault(t =>
                        t.Equals("QUAL_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase))
                    ?? tables.FirstOrDefault(t =>
                        t.Equals("dbo_CESM_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase))
                    ?? tables.FirstOrDefault(t =>
                        t.EndsWith("QUAL_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase))
                    ?? tables.FirstOrDefault(t =>
                        t.Contains("QUAL_VALIDATION_DETAIL", StringComparison.OrdinalIgnoreCase));
        ...
        public async Task<Rule31ColumnSelectionResult> GetColumnsAsync(string server, string database, string driver, string tableName)
        {
            try
            {
                ...
                return new Rule31ColumnSelectionResult
                {
                    Success = true,
                    Columns = columns,
        ...
        public async Task<Rule31FilterValueResult> GetFilterValuesAsync(string server, string database, string driver, string tableName, string errorTypeColumn)
        {
            try
            {
...
ORDER BY COUNT(*) DESC, FilterValue ASC;";
                ...
                {
                    options.Insert(0, new Rule31FilterValueOption
                    {
                        Value = "Fatal",
                        Count = 0,
                ...
                return new Rule31FilterValueResult
                {
                    Success = true,
                    Options = options,
        ...
        public async Task<Rule31VerifyResult> VerifyTableAsync(Rule31VerifyRequest request)
        {
            try
            {
...
WHERE UPPER(LTRIM(RTRIM(ISNULL(CAST([{safeErrorTypeColumn}] AS nvarchar(255)), '')))) = UPPER(@FilterValue);";
        ...
        public async Task<Rule31ValidationSummary> RunValidationAsync(Rule31ValidationRequest request, string? userEmail = null, string? userName = null)
        {
            try
            {
                ValidateRequest(request);
            ...
            {
                return new Rule31ValidationSummary
                {
                    Success = false,
                    Error = ex.Message
                };
        ...
        public async Task<Rule31WorkspaceStateViewModel?> GetCurrentWorkspaceStateAsync(int clientId, string? currentUserEmail = null, bool includeSummary = true)
        {
            await using var connection = await OpenSystemConnectionAsync();
...
ORDER BY vr.RunTimestamp DESC, vr.RunID DESC;";
            ...
            var workspace = new Rule31WorkspaceStateViewModel
            {
        ...
        public async Task<Rule31RunReviewViewModel?> GetSavedRunAsync(int runId, string? currentUserEmail = null)
        {
            await using var connection = await OpenSystemConnectionAsync();
            var currentUserId = await GetSystemUserIdByEmailAsync(connection, currentUserEmail);
...
WHERE vr.RunID = @RunID
  AND vr.RuleNumber = 31;";
            ...
            var viewModel = new Rule31RunReviewViewModel
            {
                RunId = reader.GetInt32(0),
                ClientId = reader.GetInt32(1),
        ...
        public async Task<Rule31WorkspaceSaveResult> SaveWorkspaceAsync(Rule31ValidationRequest request, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                if (request.RunId is null || request.RunId <= 0)
                {
                    return new Rule31WorkspaceSaveResult
                    {
                        Success = false,
...
WHERE RunID = @RunID
  AND ClientID = @ClientID;";
        ...
        public async Task<Rule31WorkspaceSaveResult> BeginWorkspaceEditAsync(int runId, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                await using var connection = await OpenSystemConnectionAsync();
...
WHERE RunID = @RunID;";
                ...
                markEdit.Parameters.AddWithValue("@RecordHash", ComputeHash($@"BeginWorkspaceEdit|Rule31|{runId}|{reviewerEmail}|{DateTime.UtcNow:o}|{previousHash}"));
...
END";
        ...
        public Task<string> GenerateSqlAsync(Rule31ValidationRequest request)
        {
            ...
            var sql = $@"-- ============================================================================
-- HEMIS RULE 31: FATAL ERRORS WITH EXCLUSIONS (QUAL)
-- ============================================================================
-- Table: [{safeTable}]
...
WHERE UPPER(LTRIM(RTRIM(ISNULL(CAST([{safeErrorTypeColumn}] AS nvarchar(255)), '')))) = UPPER(@FilterValue);";
            ...
            await using var reader = await cmd.ExecuteReaderAsync();
            var excludedRows = new List<Rule31ValidationRowRecord>();
            var remainingRows = new List<Rule31ValidationRowRecord>();
            ...
            while (await reader.ReadAsync())
            {
                ...
                var row = new Rule31ValidationRowRecord
                {
            ...
            return new Rule31ValidationSummary
            {
                ...
                ExcludedBreakdown = excludedBreakdown
                    ...
                    .Select(x => new Rule31BreakdownItemViewModel { ErrorCode = x.Key, Count = x.Value })
                    .ToList(),
        ...
        private async Task<int> SaveValidationRunAsync(Rule31ValidationRequest request, Rule31ValidationSummary summary, string? userEmail, string? userName)
        {
            await using var connection = await OpenSystemConnectionAsync();
            await EnsureClientNotArchivedAsync(connection, request.ClientId);
            await MarkPreviousRunsHistoricalAsync(connection, request.ClientId, 31);
            ...
            var previousHash = await GetLatestValidationHashAsync(connection, request.ClientId, 31);
...
VALUES
(
    @ClientID, @UserID, 31, @RuleName, @Status, @TotalRecords, @PassCount, @FailCount, @ExceptionRate, GETDATE(),
    @HemisServer, @AuditDatabase, @StudTable, @DeceasedTable, @StudColumn, @DeceasedColumn,
...
SELECT CAST(SCOPE_IDENTITY() AS int);";
            ...
            command.Parameters.AddWithValue("@RuleName", "Fatal Errors with Exclusions (QUAL)");
...
WHERE RunID = @RunID;";
            ...
            hashCommand.Parameters.AddWithValue("@RecordHash", ComputeHash($@"ValidationRun|Rule31|{runId}|{request.ClientId}|{systemUserId.Value}|{summary.Status}|{summary.TotalValidated}|{summary.FailCount}|{summary.ExceptionRate}|{summary.Timestamp}|{previousHash}"));
...
ORDER BY ORDINAL_POSITION;";
        ...
        private static void ValidateRequest(Rule31ValidationRequest request)
        {
        ...
        private static void ValidateRequest(Rule31VerifyRequest request)
        {
            ValidateRequest(new Rule31ValidationRequest
            {
...
ORDER BY RunTimestamp DESC, RunID DESC;";
        ...
        private async Task<Rule31ValidationSummary?> GetValidationSummaryAsync(SqlConnection connection, int runId)
        {
            await using var command = connection.CreateConfiguredCommand();
        ...
        private static Rule31ValidationSummary? DeserializeSummary(string? json)
        {
            ...
            try
            {
                ...
                return JsonConvert.DeserializeObject<Rule31ValidationSummary>(decoded);
            }
            catch
            {
                return null;
        ...
        private static void ApplyBrowserPreview(Rule31ValidationSummary summary)
        {
