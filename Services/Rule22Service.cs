        ...
        public async Task<Rule22WorkspaceSaveResult> SaveWorkspaceAsync(Rule22ValidationRequest request, string reviewerEmail, string? reviewerName = null)
        {
            try
            {
                ...
                if (existingSummary != null)
                {
                    ...
                    existingSummary.ProfTable = request.ProfTable;
                    existingSummary.Column041 = request.Column041;
                    existingSummary.Column039 = request.Column039;
                    existingSummary.Control1SampleSize = request.Control1SampleSize;
                    existingSummary.Control2SampleSize = request.Control2SampleSize;
                    existingSummary.Control3SampleSize = request.Control3SampleSize;
...
SELECT CAST(SCOPE_IDENTITY() AS int);";
            ...
            command.Parameters.AddWithValue("@ExceptionsJSON", JsonConvert.SerializeObject(failRows));
...
ORDER BY RunTimestamp DESC, RunID DESC;";
    ...
    END";
        ...
        private static Rule22ValidationSummary? DeserializeSummary(string? json)
        {
            ...
            try
            {
                var summary = JsonConvert.DeserializeObject<Rule22ValidationSummary>(json);
