...
namespace HemisAudit.Services
{
    public class Rule36Service : IRule36Service
    {
        ...
        public async Task<DatabaseListResult> GetDatabasesAsync(string server, string driver)
        {
            try
            {
                var connStr = $"Server={server};Database=master;Trusted_Connection=True;TrustServerCertificate=True;Encrypt=False;Connection Timeout=30;";
...
WHERE vr.RunID = @RunID;";
            ...
            var summaryJson = reader.IsDBNull(5) ? null : reader.GetString(5);
            var summary = DeserializeSummary(summaryJson);
            if (summary == null)
                return null;
...
WHERE ClientID = @ClientID;";
        ...
        private static ValidationSummary? DeserializeSummary(string? json)
        {
            ...
            try
            {
                return JsonConvert.DeserializeObject<ValidationSummary>(json);
            }
            catch
            {
                return null;
