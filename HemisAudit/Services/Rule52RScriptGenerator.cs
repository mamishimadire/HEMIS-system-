...
public static class Rule52RScriptGenerator
{
    private static string RString(string? value) =>
        (value ?? "").Replace("\\", "\\\\").Replace("'", "\\'");

    public static string Generate(Rule52ValidationRequest req)
    {
        var valpacTable  = RString(req.ValpacTable);
        var prodTable    = RString(req.ProdTable);
        var valpacKeyCol = RString(req.ValpacSubjCol);
        var prodKeyCol   = RString(req.ProdSubjCol);
...
cat(sprintf('-- Rule 52: QUAL VALPAC Data vs QUAL PRODUCTION --\n'))
