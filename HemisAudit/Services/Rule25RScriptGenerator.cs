...
public static class Rule25RScriptGenerator
{
    private static string RString(string? value) =>
        (value ?? "").Replace("\\", "\\\\").Replace("'", "\\'");

    public static string Generate(Rule25ValidationRequest req)
    {
        var table1 = RString(req.CrseTable);
...
cat(sprintf('-- Rule 25: Reconcile Course Datasets --\n'))
