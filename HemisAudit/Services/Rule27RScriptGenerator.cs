...
public static class Rule27RScriptGenerator
{
    private static string RString(string? value) =>
        (value ?? "").Replace("\\", "\\\\").Replace("'", "\\'");

    public static string Generate(Rule27ValidationRequest req)
    {
...
if (length(err_cols) == 0) {{
  ...
  cat(sprintf('-- Rule 27: Error Filter --\n'))
