...
public static class Rule31RScriptGenerator
{
    private static string RString(string? value) =>
        (value ?? "").Replace("\\", "\\\\").Replace("'", "\\'");

    public static string Generate(Rule31ValidationRequest req)
    {
...
if (length(err_cols) == 0) {{
  ...
  cat(sprintf('-- Rule 31: Error Filter --\n'))
