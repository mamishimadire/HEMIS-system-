...
public static class Rule51RScriptGenerator
{
    private static string RString(string? value) =>
        (value ?? "").Replace("\\", "\\\\").Replace("'", "\\'");

    public static string Generate(Rule51ValidationRequest req)
    {
        var valpacTable  = RString(req.ValpacTable);
        var prodTable    = RString(req.ProdTable);
        var valpacKeyCol = RString(req.ValpacCol007);
        var prodKeyCol   = RString(req.ProdColStNo);
...
cat(sprintf('-- Rule 51: Student VALPAC Data vs STUD PRODUCTION --\n'))
