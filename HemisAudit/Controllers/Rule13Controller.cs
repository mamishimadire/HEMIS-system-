...
{
    [Authorize]
    public class Rule13Controller : Controller
    {
        private readonly IRule13Service _rule13;
        private readonly IExportService _export;
        private readonly IAuditLogService _audit;
        private readonly UserManager<ApplicationUser> _users;
        private readonly ISystemDatabaseService _systemDb;
        private readonly ILogger<Rule13Controller> _logger;

        public Rule13Controller(
            IRule13Service rule13,
            IExportService export,
            IAuditLogService audit,
            UserManager<ApplicationUser> users,
            ISystemDatabaseService systemDb,
            ILogger<Rule13Controller> logger)
        {
            _rule13 = rule13;
            _export = export;
            _audit = audit;
        ...
        public async Task<IActionResult> Index(int clientId = 0)
        {
            ...
            ViewBag.ModuleNavigation = ModuleSequenceNavigationHelper.BuildForWorkspace(13, clientId);
            return View();
        }
        ...
        public async Task<IActionResult> GetWorkspaceState(int clientId)
        {
            ...
            var workspace = await _rule13.GetCurrentWorkspaceStateAsync(clientId, user?.Email);
        ...
        public async Task<IActionResult> Run(int id)
        {
            ...
            var review = await _rule13.GetSavedRunAsync(id, user?.Email);
            if (review == null)
                return NotFound();
        ...
        public async Task<IActionResult> VerifyTables([FromBody] Rule13VerifyRequest request) =>
            Json(await RequireDataAnalystAsync(async () => await _rule13.VerifyTablesAsync(request)));

        [HttpPost]
        public async Task<IActionResult> RunValidation([FromBody] Rule13ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                _logger.LogWarning("Rule13 rejected because no client was selected.");
                return Json(new Rule13ValidationSummary
                {
                    Success = false,
            ...
            {
                _logger.LogWarning("Rule13 rejected for {Email} because the user cannot access client {ClientId}.", user?.Email, request.ClientId);
                return Json(new Rule13ValidationSummary
                {
                    Success = false,
            ...
            {
                _logger.LogWarning(
                    "Rule13 rejected for {Email}. SystemRole={SystemRole}, EngagementRole={EngagementRole}, ClientId={ClientId}",
                    user?.Email,
                    role,
                    engagementRole,
                    request.ClientId);
                ...
                {
                    Success = false,
                    Error = "Only the assigned data analyst can run Rule 13."
                });
            }

            async Task<Rule13ValidationSummary> ExecuteValidationAsync(IRule13Service ruleService, IAuditLogService auditService)
            {
                var result = await ruleService.RunValidationAsync(request, user?.Email, user?.FullName ?? user?.Email);
                _logger.LogInformation(
                    "Rule13 RunValidation completed for {Email}. Success={Success}, Status={Status}, Error={Error}, TotalValidated={TotalValidated}, SavedRunId={SavedRunId}",
                    user?.Email,
                    result.Success,
                    result.Status,
                    result.Error,
                ...
                if (result.Success)
                {
                    await auditService.LogAsync(
                        "run_validation",
                        $"Rule 13 on client {request.ClientId}: {result.Status} ({result.FailCount} fail rows), run {result.SavedRunId}",
                        user?.Id,
                        user?.Email);
                }
            ...
            if (ValidationOperationHttpHelper.IsAsyncRequested(Request))
            {
                return ValidationOperationHttpHelper.Queue(
                    ...
                    async (sp, ct) => await ExecuteValidationAsync(
                        sp.GetRequiredService<IRule13Service>(),
                        sp.GetRequiredService<IAuditLogService>()));
            }

            var result = await ExecuteValidationAsync(_rule13, _audit);
            return Json(result);
        }

        [HttpPost]
        public async Task<IActionResult> BeginWorkspaceEdit([FromBody] Rule13ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                return Json(new Rule13WorkspaceSaveResult
                {
                    Success = false,
            ...
            var result = await _rule13.BeginWorkspaceEditAsync(request.RunId.Value, user!.Email!, user.FullName);
            if (result.Success)
            {
                await _audit.LogAsync(
                    "workspace_edit_started",
                    $"DataAnalyst started editing Rule 13 run {request.RunId.Value}. Existing signoffs were cleared.",
                    user?.Id,
                    user?.Email);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> SaveWorkspace([FromBody] Rule13ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            var result = await _rule13.SaveWorkspaceAsync(request, user!.Email!, user.FullName);
            if (result.Success)
            {
                await _audit.LogAsync(
                    "save_validation_workspace",
                    $"DataAnalyst saved Rule 13 workspace for client {request.ClientId}. Signoffs cleared: {result.ClearedSignoffCount ?? 0}",
                    user?.Id,
                    user?.Email);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> GenerateSql([FromBody] Rule13ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                return Json(new Rule13SqlResult
                {
                    Success = false,
                    Error = "You cannot access this engagement."
                });
            ...
            return Json(await RequireDataAnalystAsync(async () =>
                ...
                {
                    Success = true,
                    Sql = await _rule13.GenerateSqlAsync(request)
                }));
        }
        [HttpPost]
        public async Task<IActionResult> GenerateRScript([FromBody] Rule13ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);

            if (request.ClientId > 0 && !await _systemDb.CanAccessClientResultsAsync(request.ClientId, user, role))
                return Json(new Rule13SqlResult { Success = false, Error = "You cannot access this engagement." });

            return Json(await RequireDataAnalystAsync(async () => new Rule13SqlResult
            {
                Success = true,
                Sql = Rule13RScriptGenerator.Generate(request) + RScriptScaffold.BuildAutoExportFooter("Rule13")
            }));
        }

        [HttpPost]
        public async Task<IActionResult> SignOffWorkspace([FromBody] Rule13WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            var review = await _rule13.GetSavedRunAsync(model.RunId.Value, user?.Email);
            ...
            try
            {
                await _rule13.AddOrUpdateSignoffAsync(model.RunId.Value, user!.Email!, model.Comment);
                await _audit.LogAsync(
                    "signoff_validation_run",
                    $"Rule 13 signoff saved for run {model.RunId.Value} from module workspace",
                    user.Id,
                    user.Email);
            }
            catch (Exception ex)
            ...
            var workspace = await _rule13.GetCurrentWorkspaceStateAsync(model.ClientId, user?.Email, includeSummary: false);
            var resultsVisible = CanViewWorkspaceResults(role, workspace);
        ...
        [HttpPost]
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule13WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            try
            {
                await _rule13.RemoveSignoffAsync(model.RunId.Value, user!.Email!);
            }
            catch (Exception ex)
            {
                return Json(new { success = false, error = ex.Message });
            ...
            await _audit.LogAsync(
                ...
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 13 run {model.RunId.Value} from module workspace. Historical snapshot preserved; new current run {reopenedRunId.Value} created for continued review."
                    : $"{review.CurrentUserEngagementRole} removed signoff for Rule 13 run {model.RunId.Value} from module workspace",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> AddSignoff(Rule13RunSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule13.GetSavedRunAsync(model.RunId, user?.Email);
            if (review == null)
                return NotFound();
            ...
            await _rule13.AddOrUpdateSignoffAsync(model.RunId, user!.Email!, model.Comment);
            await _audit.LogAsync(
                "signoff_validation_run",
                $"{review.CurrentUserEngagementRole} signed off Rule 13 run {model.RunId}",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule13.GetSavedRunAsync(runId, user?.Email);
            if (review == null)
                return NotFound();
            ...
            await _rule13.RemoveSignoffAsync(runId, user!.Email!);
            var workspace = await _rule13.GetCurrentWorkspaceStateAsync(review.ClientId, user?.Email, includeSummary: false);
            var redirectRunId = workspace?.RunId ?? runId;
            var preservedHistory = workspace?.RunId.HasValue == true && workspace.RunId.Value != runId;
            await _audit.LogAsync(
                "remove_validation_signoff",
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 13 run {runId}. Historical snapshot preserved; new current run {redirectRunId} created for continued review."
                    : $"{review.CurrentUserEngagementRole} removed signoff for Rule 13 run {runId}",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            ...
            return File(bytes, "text/csv", $"Rule13_CESM_Qualification_Population_Run_{runId}.csv");
        }
        ...
        public async Task<IActionResult> DownloadSavedSql(int runId)
        {
            ...
            return File(bytes, "application/sql", $"Rule13_CESM_Qualification_Population_{runId}.sql");
        }
        ...
        [HttpPost]
        public async Task<IActionResult> DownloadCsv([FromBody] Rule13ValidationSummary summary)
        {
            summary = await ResolveExportSummaryAsync(summary);
            var fileName = $"Rule13_CESM_Qualification_Population_{Ts()}.csv";
            var bytes = _export.ExportCsv(summary);
            return File(bytes, "text/csv", fileName);
        }

        [HttpPost]
        public async Task<IActionResult> DownloadSql([FromBody] Rule13ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            ...
            var fileName = $"Rule13_CESM_Qualification_Population_{Ts()}.sql";
            var bytes = _export.ExportSql(await _rule13.GenerateSqlAsync(request));
            return File(bytes, "application/sql", fileName);
        }

        private async Task<Rule13RunReviewViewModel?> LoadAuthorizedSavedRunAsync(int runId, bool requireDownloadAccess)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule13.GetSavedRunAsync(runId, user?.Email, includeFullResults: requireDownloadAccess);
            if (review == null)
            {
        ...
        private static bool CanDownloadSavedRun(Rule13RunReviewViewModel review, string systemRole)
        ...
        private static bool CanViewSavedRun(Rule13RunReviewViewModel review, string systemRole)
        ...
        private static bool CanViewWorkspaceResults(string role, Rule13WorkspaceStateViewModel? workspace)
        {
            if (workspace == null)
                return false;
        ...
        private async Task<Rule13ValidationSummary> ResolveExportSummaryAsync(Rule13ValidationSummary summary)
        {
            ...
            {
                var review = await _rule13.GetSavedRunAsync(savedRunId, user?.Email, includeFullResults: true);
            ...
            {
                var workspace = await _rule13.GetCurrentWorkspaceStateAsync(summary.ClientId, user?.Email, includeSummary: false);
                if (workspace?.RunId is int workspaceRunId && workspaceRunId > 0)
                {
                    var review = await _rule13.GetSavedRunAsync(workspaceRunId, user?.Email, includeFullResults: true);
                    if (review?.Summary != null)
                    {
                        return review.Summary;
                    }
        ...
        private async Task<object> RequireDataAnalystAsync<T>(Func<Task<T>> action)
        {
            ...
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            {
                return new { success = false, error = "Only the assigned data analyst can configure or run Rule 13." };
            }
