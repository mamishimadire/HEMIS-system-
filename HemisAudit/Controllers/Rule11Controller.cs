...
{
    [Authorize]
    public class Rule11Controller : Controller
    {
        private readonly IRule11Service _rule11;
        private readonly IExportService _export;
        private readonly IAuditLogService _audit;
        private readonly UserManager<ApplicationUser> _users;
        private readonly ISystemDatabaseService _systemDb;

        public Rule11Controller(
            IRule11Service rule11,
        ...
        public async Task<IActionResult> Index(int clientId = 0)
        {
            ...
            ViewBag.ModuleNavigation = ModuleSequenceNavigationHelper.BuildForWorkspace(11, clientId);
            return View();
        }
        ...
        public async Task<IActionResult> GetWorkspaceState(int clientId)
        {
            ...
            var workspace = await _rule11.GetCurrentWorkspaceStateAsync(clientId, user?.Email);
        ...
        public async Task<IActionResult> Run(int id)
        {
            ...
            var role = await GetCurrentSystemRoleAsync(user);
            await _systemDb.NormalizeCompletedRunStatusesAsync();
            var review = await _rule11.GetSavedRunAsync(id, user?.Email);
            if (review == null)
                return NotFound();
            ...
            ViewBag.ModuleNavigation = ModuleSequenceNavigationHelper.BuildForSavedRun(
                11, review.ClientId, clientDetail?.ValidationRuns, role, review.CurrentUserEngagementRole);
            ...
            review.GeneratedSql = _rule11.GenerateSql(new Rule11ValidationRequest
            {
                ...
                QualHeqfTypeCol = review.Summary.QualHeqfTypeCol,
                ...
                PqmHeqfTypeCol  = review.Summary.PqmHeqfTypeCol,
                PqmCodeCol      = review.Summary.PqmCodeCol
            });
            return View(review);
        }
        ...
        public async Task<IActionResult> GetDatabases([FromBody] ConnectionViewModel model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule11.GetDatabasesAsync(model.Server, model.Driver)));

        [HttpPost]
        public async Task<IActionResult> GetTables([FromBody] ConnectionViewModel model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule11.GetTablesAsync(model.Server, model.Database, model.Driver)));

        [HttpPost]
        public async Task<IActionResult> GetColumns([FromBody] Rule11GetColumnsRequest model) =>
            Json(await RequireDataAnalystAsync(async () =>
                await _rule11.GetColumnsAsync(model.Server, model.Database, model.Driver, model.TableName, model.TableRole)));

        [HttpPost]
        public async Task<IActionResult> GetFilterValues([FromBody] Rule11FilterValueRequest model) =>
            Json(await RequireDataAnalystAsync(async () =>
                await _rule11.GetFilterValuesAsync(model.Server, model.Database, model.Driver, model.QualTable, model.ApprovalColumn)));

        [HttpPost]
        public async Task<IActionResult> VerifyTables([FromBody] Rule11VerifyRequest request) =>
            Json(await RequireDataAnalystAsync(async () => await _rule11.VerifyDataAsync(request)));

        [HttpPost]
        public async Task<IActionResult> RunValidation([FromBody] Rule11ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);

            if (request.ClientId <= 0)
                return Json(new Rule11ValidationSummary { Success = false, Error = "Select an approved engagement before running validation." });

            if (!await _systemDb.CanAccessClientResultsAsync(request.ClientId, user, role))
                return Json(new Rule11ValidationSummary { Success = false, Error = "You cannot access this engagement." });
            ...
            {
                return Json(new Rule11ValidationSummary { Success = false, Error = "Only the assigned data analyst can run Rule 11." });
            }

            async Task<Rule11ValidationSummary> ExecuteAsync(IRule11Service svc, IAuditLogService auditSvc)
            {
                ...
                if (result.Success)
                {
                    await auditSvc.LogAsync(
                        "run_validation",
                        $"Rule 11 on client {request.ClientId}: {result.Status} ({result.FailCount} exceptions), run {result.SavedRunId}",
                        user?.Id, user?.Email);
                }
                return result;
            }
            ...
            if (ValidationOperationHttpHelper.IsAsyncRequested(Request))
            {
                return ValidationOperationHttpHelper.Queue(
                    ...
                    async (sp, ct) => await ExecuteAsync(
                        sp.GetRequiredService<IRule11Service>(),
                        sp.GetRequiredService<IAuditLogService>()));
            }

            return Json(await ExecuteAsync(_rule11, _audit));
        }

        [HttpPost]
        public async Task<IActionResult> BeginWorkspaceEdit([FromBody] Rule11ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);

            if (!await CanEditWorkspaceAsync(request.ClientId, user, role))
                return Json(new Rule11WorkspaceSaveResult { Success = false, Error = "Only the assigned data analyst can edit a saved workspace." });

            if (!request.RunId.HasValue || request.RunId.Value <= 0)
                return Json(new Rule11WorkspaceSaveResult { Success = false, Error = "Select a saved run before editing the workspace." });

            var result = await _rule11.BeginWorkspaceEditAsync(request.RunId.Value, user!.Email!, user.FullName);
            if (result.Success)
            {
                await _audit.LogAsync("workspace_edit_started",
                    $"DataAnalyst started editing Rule 11 run {request.RunId.Value}. Existing signoffs were cleared.",
                    user?.Id, user?.Email);
            }
            return Json(result);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> SaveWorkspace([FromBody] Rule11ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);

            if (!await CanEditWorkspaceAsync(request.ClientId, user, role))
                return Json(new Rule11WorkspaceSaveResult { Success = false, Error = "Only the assigned data analyst can save a workspace." });

            var result = await _rule11.SaveWorkspaceAsync(request, user!.Email!, user.FullName);
            if (result.Success)
            {
                await _audit.LogAsync("save_validation_workspace",
                    $"DataAnalyst saved Rule 11 workspace for client {request.ClientId}. Signoffs cleared: {result.ClearedSignoffCount ?? 0}",
                    user?.Id, user?.Email);
            }
            return Json(result);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> SignOffWorkspace([FromBody] Rule11WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            var review = await _rule11.GetSavedRunAsync(model.RunId.Value, user?.Email);
            ...
            try
            {
                await _rule11.AddOrUpdateSignoffAsync(model.RunId.Value, user!.Email!, model.Comment);
                await _audit.LogAsync("signoff_validation_run",
                    $"Signed off Rule 11 run {model.RunId.Value} from module workspace",
                    user.Id, user.Email);
            }
            catch (Exception ex)
            {
            ...
            var workspace = await _rule11.GetCurrentWorkspaceStateAsync(model.ClientId, user?.Email);
            var resultsVisible = CanViewWorkspaceResults(role, workspace);
        ...
        [HttpPost]
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule11WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            try
            {
                await _rule11.RemoveSignoffAsync(model.RunId.Value, user!.Email!);
            }
            catch (Exception ex)
            {
                return Json(new { success = false, error = ex.Message });
            ...
            await _audit.LogAsync("remove_validation_signoff",
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 11 run {model.RunId.Value}. Historical snapshot preserved; new current run {reopenedRunId.Value} created."
                    : $"{review.CurrentUserEngagementRole} removed signoff for Rule 11 run {model.RunId.Value} from module workspace",
                user.Id, user.Email);
            return Json(new { success = true, message, resultsVisible, workspace });
        }

        [HttpPost]
        public async Task<IActionResult> GenerateSql([FromBody] Rule11ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            return Json(RequireDataAnalystResult(() => new SqlResult { Success = true, Sql = _rule11.GenerateSql(request) }));
        }
        [HttpPost]
        public async Task<IActionResult> GenerateRScript([FromBody] Rule11ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                Success = true,
                Sql = Rule11RScriptGenerator.Generate(request) + RScriptScaffold.BuildAutoExportFooter("Rule11")
            }));
        }

        [HttpPost, ValidateAntiForgeryToken]
        public async Task<IActionResult> AddSignoff(Rule11RunSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule11.GetSavedRunAsync(model.RunId, user?.Email);
            ...
            await _rule11.AddOrUpdateSignoffAsync(model.RunId, user!.Email!, model.Comment);
            await _audit.LogAsync("signoff_validation_run",
                $"{review.CurrentUserEngagementRole} signed off Rule 11 run {model.RunId}",
                user.Id, user.Email);
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule11.GetSavedRunAsync(runId, user?.Email);
            if (review == null) return NotFound();
            ...
            await _rule11.RemoveSignoffAsync(runId, user!.Email!);
            var workspace = await _rule11.GetCurrentWorkspaceStateAsync(review.ClientId, user?.Email);
            var redirectRunId = workspace?.RunId ?? runId;
            var preservedHistory = workspace?.RunId.HasValue == true && workspace.RunId.Value != runId;
            await _audit.LogAsync("remove_validation_signoff",
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 11 run {runId}. Historical snapshot preserved; new current run {redirectRunId} created."
                    : $"{review.CurrentUserEngagementRole} removed signoff for Rule 11 run {runId}",
                user.Id, user.Email);
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            var review = await LoadAuthorizedSavedRunAsync(runId, requireDownloadAccess: true);
            if (review == null) return RedirectToAction(nameof(Run), new { id = runId });
            var bytes = _export.ExportRule11Csv(review.Summary, false);
            return File(bytes, "text/csv", $"Rule11_Validation_Results_Run_{runId}.csv");
        }
        ...
        public async Task<IActionResult> DownloadSavedExceptionsCsv(int runId)
        {
            var review = await LoadAuthorizedSavedRunAsync(runId, requireDownloadAccess: true);
            if (review == null) return RedirectToAction(nameof(Run), new { id = runId });
            var bytes = _export.ExportRule11Csv(review.Summary, true);
            return File(bytes, "text/csv", $"Rule11_QUAL_CESM_PQM_Exceptions_Run_{runId}.csv");
        }
        ...
        public async Task<IActionResult> DownloadSavedSql(int runId)
        {
            var review = await LoadAuthorizedSavedRunAsync(runId, requireDownloadAccess: true);
            if (review == null) return RedirectToAction(nameof(Run), new { id = runId });
            var request = new Rule11ValidationRequest
            {
                ClientId        = review.ClientId,
                Database        = review.Summary.Database,
                QualTable       = review.Summary.QualTable,
            ...
            var bytes = _export.ExportSql(_rule11.GenerateSql(request));
            return File(bytes, "application/sql", $"Rule11_QUAL_CESM_PQM_Validation_Run_{runId}.sql");
        }
        ...
        [HttpPost]
        public async Task<IActionResult> DownloadCsv([FromBody] Rule11ValidationSummary summary)
        {
            summary = await ResolveExportSummaryAsync(summary);
            var bytes = _export.ExportRule11Csv(summary, false);
            return File(bytes, "text/csv", $"Rule11_Validation_Results_{Ts()}.csv");
        }

        [HttpPost]
        public async Task<IActionResult> DownloadExceptionsCsv([FromBody] Rule11ValidationSummary summary)
        {
            summary = await ResolveExportSummaryAsync(summary);
            var bytes = _export.ExportRule11Csv(summary, true);
            return File(bytes, "text/csv", $"Rule11_QUAL_CESM_PQM_Exceptions_{Ts()}.csv");
        }

        [HttpPost]
        public async Task<IActionResult> DownloadSql([FromBody] Rule11ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            ...
            var bytes = _export.ExportSql(_rule11.GenerateSql(request));
            return File(bytes, "application/sql", $"Rule11_QUAL_CESM_PQM_Validation_{Ts()}.sql");
        }
        ...
        private async Task<Rule11RunReviewViewModel?> LoadAuthorizedSavedRunAsync(int runId, bool requireDownloadAccess)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule11.GetSavedRunAsync(runId, user?.Email);
        ...
        private static bool CanDownloadSavedRun(Rule11RunReviewViewModel review, string systemRole)
        ...
        private static bool CanViewSavedRun(Rule11RunReviewViewModel review, string systemRole)
            => ValidationRunAccessPolicy.CanViewSignedResults(systemRole, review.CurrentUserEngagementRole, review.HasDataAnalystSignoff);

        private static bool CanViewWorkspaceResults(string role, Rule11WorkspaceStateViewModel? workspace)
        {
        ...
        private async Task<Rule11ValidationSummary> ResolveExportSummaryAsync(Rule11ValidationSummary summary)
        {
            var user = await _users.GetUserAsync(User);
            if (summary.SavedRunId is int savedRunId && savedRunId > 0)
            {
                var review = await _rule11.GetSavedRunAsync(savedRunId, user?.Email);
                if (review?.Summary != null) return review.Summary;
            }
            if (summary.ClientId > 0)
            {
                var workspace = await _rule11.GetCurrentWorkspaceStateAsync(summary.ClientId, user?.Email);
                if (workspace?.RunId is int workspaceRunId && workspaceRunId > 0)
                {
                    var review = await _rule11.GetSavedRunAsync(workspaceRunId, user?.Email);
                    if (review?.Summary != null) return review.Summary;
                }
            }
            return summary;
        ...
        private async Task<object> RequireDataAnalystAsync<T>(Func<Task<T>> action)
        {
            ...
            var role = await GetCurrentSystemRoleAsync(user);
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
                return new { success = false, error = "Only the assigned data analyst can configure or run Rule 11." };
            return await action();
        }
