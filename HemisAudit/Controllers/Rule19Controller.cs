...
{
    [Authorize]
    public class Rule19Controller : Controller
    {
        private readonly IRule19Service _rule19;
        private readonly IExportService _export;
        private readonly IAuditLogService _audit;
        private readonly UserManager<ApplicationUser> _users;
        private readonly ISystemDatabaseService _systemDb;

        public Rule19Controller(
            IRule19Service rule19,
        ...
        public async Task<IActionResult> Index(int clientId = 0)
        {
            ...
            ViewBag.ModuleNavigation = ModuleSequenceNavigationHelper.BuildForWorkspace(19, clientId);
            return View();
        }
        ...
        public async Task<IActionResult> GetWorkspaceState(int clientId)
        {
            ...
            var workspace = await _rule19.GetCurrentWorkspaceStateAsync(clientId, user?.Email);
        ...
        public async Task<IActionResult> Run(int id)
        {
            ...
            var review = await _rule19.GetSavedRunAsync(id, user?.Email);
            if (review == null)
                return NotFound();
            ...
            try
            {
                review.GeneratedSql = await _rule19.GenerateSqlAsync(new Rule19ValidationRequest
                {
        ...
        public async Task<IActionResult> GetDatabases([FromBody] ConnectionViewModel model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule19.GetDatabasesAsync(model.Server, model.Driver)));

        [HttpPost]
        public async Task<IActionResult> GetTables([FromBody] ConnectionViewModel model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule19.GetTablesAsync(model.Server, model.Database, model.Driver)));

        [HttpPost]
        public async Task<IActionResult> GetColumns([FromBody] Rule19GetColumnsRequest model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule19.GetColumnsAsync(model.Server, model.Database, model.Driver, model.TableName)));

        [HttpPost]
        public async Task<IActionResult> LoadFilterValues([FromBody] Rule19FilterValueRequest model) =>
            Json(await RequireDataAnalystAsync(async () =>
                await _rule19.GetFilterValuesAsync(model.Server, model.Database, model.Driver, model.StudTable, model.FulfilledColumn)));

        [HttpPost]
        public async Task<IActionResult> VerifyTables([FromBody] Rule19VerifyRequest request) =>
            Json(await RequireDataAnalystAsync(async () => await _rule19.VerifyTablesAsync(request)));

        [HttpPost]
        public async Task<IActionResult> RunValidation([FromBody] Rule19ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                return Json(new Rule19ValidationSummary
                {
                    Success = false,
                ...
                !string.Equals(engagementRole, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    Success = false,
                    Error = "Only the assigned data analyst can run Rule 19."
                });
            }

            async Task<Rule19ValidationSummary> ExecuteValidationAsync(IRule19Service ruleService, IAuditLogService auditService)
            {
                ...
                if (result.Success)
                {
                    await auditService.LogAsync(
                        "run_validation",
                        $"Rule 19 on client {request.ClientId}: {result.Status} ({result.MatchingCount} population rows), run {result.SavedRunId}",
                        user?.Id,
                        user?.Email);
                }
            ...
            if (ValidationOperationHttpHelper.IsAsyncRequested(Request))
            {
                return ValidationOperationHttpHelper.Queue(
                    ...
                    async (sp, ct) => await ExecuteValidationAsync(
                        sp.GetRequiredService<IRule19Service>(),
                        sp.GetRequiredService<IAuditLogService>()));
            }

            var result = await ExecuteValidationAsync(_rule19, _audit);
            return Json(result);
        }

        [HttpPost]
        public async Task<IActionResult> BeginWorkspaceEdit([FromBody] Rule19ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                return Json(new Rule19WorkspaceSaveResult
                {
                    Success = false,
            ...
            var result = await _rule19.BeginWorkspaceEditAsync(request.RunId.Value, user!.Email!, user.FullName);
            if (result.Success)
            {
                await _audit.LogAsync(
                    "workspace_edit_started",
                    $"DataAnalyst started editing Rule 19 run {request.RunId.Value}. Existing signoffs were cleared.",
                    user?.Id,
                    user?.Email);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> SaveWorkspace([FromBody] Rule19ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            var result = await _rule19.SaveWorkspaceAsync(request, user!.Email!, user.FullName);
            if (result.Success)
            {
                await _audit.LogAsync(
                    "save_validation_workspace",
                    $"DataAnalyst saved Rule 19 workspace for client {request.ClientId}. Signoffs cleared: {result.ClearedSignoffCount ?? 0}",
                    user?.Id,
                    user?.Email);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> SignOffWorkspace([FromBody] Rule19WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            if (!model.RunId.HasValue || model.RunId.Value <= 0)
                return Json(new { success = false, error = "Run Rule 19 first so the workspace is saved." });

            var review = await _rule19.GetSavedRunAsync(model.RunId.Value, user?.Email);
            ...
            try
            {
                await _rule19.AddOrUpdateSignoffAsync(model.RunId.Value, user!.Email!, model.Comment);
                await _audit.LogAsync(
                    "signoff_validation_run",
                    $"Rule 19 signoff saved for run {model.RunId.Value} from module workspace",
                    user.Id,
                    user.Email);
            }
            catch (Exception ex)
            ...
            var workspace = await _rule19.GetCurrentWorkspaceStateAsync(model.ClientId, user?.Email, includeSummary: false);
            var resultsVisible = CanViewWorkspaceResults(role, workspace);
        ...
        [HttpPost]
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule19WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            try
            {
                await _rule19.RemoveSignoffAsync(model.RunId.Value, user!.Email!);
            }
            catch (Exception ex)
            {
                return Json(new { success = false, error = ex.Message });
            ...
            await _audit.LogAsync(
                ...
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 19 run {model.RunId.Value} from module workspace. Historical snapshot preserved; new current run {reopenedRunId.Value} created for continued review."
                    : $"{review.CurrentUserEngagementRole} removed signoff for Rule 19 run {model.RunId.Value} from module workspace",
                user.Id,
                user.Email);
        ...
        [HttpPost]
        public async Task<IActionResult> GenerateSql([FromBody] Rule19ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                return Json(new Rule19SqlResult
                {
                    Success = false,
                    Error = "You cannot access this engagement."
                });
            ...
            return Json(await RequireDataAnalystAsync(async () =>
                ...
                {
                    Success = true,
                    Sql = await _rule19.GenerateSqlAsync(request)
                }));
        }
        [HttpPost]
        public async Task<IActionResult> GenerateRScript([FromBody] Rule19ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);

            if (request.ClientId > 0 && !await _systemDb.CanAccessClientResultsAsync(request.ClientId, user, role))
                return Json(new Rule19SqlResult { Success = false, Error = "You cannot access this engagement." });

            return Json(await RequireDataAnalystAsync(async () => new Rule19SqlResult
            {
                Success = true,
                Sql = Rule19RScriptGenerator.Generate(request) + RScriptScaffold.BuildAutoExportFooter("Rule19")
            }));
        }

        [HttpPost, ValidateAntiForgeryToken]
        public async Task<IActionResult> AddSignoff(Rule19RunSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule19.GetSavedRunAsync(model.RunId, user?.Email);
            if (review == null)
                return NotFound();
            ...
            await _rule19.AddOrUpdateSignoffAsync(model.RunId, user!.Email!, model.Comment);
            await _audit.LogAsync(
                "signoff_validation_run",
                $"{review.CurrentUserEngagementRole} signed off Rule 19 run {model.RunId}",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule19.GetSavedRunAsync(runId, user?.Email);
            if (review == null)
                return NotFound();
            ...
            await _rule19.RemoveSignoffAsync(runId, user!.Email!);
            var workspace = await _rule19.GetCurrentWorkspaceStateAsync(review.ClientId, user?.Email, includeSummary: false);
            var redirectRunId = workspace?.RunId ?? runId;
            var preservedHistory = workspace?.RunId.HasValue == true && workspace.RunId.Value != runId;
            await _audit.LogAsync(
                "remove_validation_signoff",
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 19 run {runId}. Historical snapshot preserved; new current run {redirectRunId} created for continued review."
                    : $"{review.CurrentUserEngagementRole} removed signoff for Rule 19 run {runId}",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> DownloadSavedExcel(int runId)
        {
            ...
            return File(bytes,
                ...
                $"Rule19_Masters_PhD_Population_Run_{runId}.xlsx");
        }
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            ...
            return File(bytes, "text/csv", $"Rule19_Masters_PhD_Population_Run_{runId}.csv");
        }
        ...
        public async Task<IActionResult> DownloadSavedSql(int runId)
        {
            ...
            var request = new Rule19ValidationRequest
            {
                ClientId = review.ClientId,
                Server = review.SourceServer,
                Database = review.Summary.Database,
            ...
            var bytes = _export.ExportSql(await _rule19.GenerateSqlAsync(request));
        ...
        [HttpPost]
        public async Task<IActionResult> DownloadSql([FromBody] Rule19ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            ...
            var bytes = _export.ExportSql(await _rule19.GenerateSqlAsync(request));
            return File(bytes, "application/sql", $"Rule19_Masters_PhD_Population_{Ts()}.sql");
        }

        private async Task<Rule19RunReviewViewModel?> LoadAuthorizedSavedRunAsync(int runId, bool requireDownloadAccess)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule19.GetSavedRunAsync(runId, user?.Email, includeFullResults: requireDownloadAccess);
            if (review == null)
            {
        ...
        private static bool CanDownloadSavedRun(Rule19RunReviewViewModel review, string systemRole)
        ...
        private static bool CanViewSavedRun(Rule19RunReviewViewModel review, string systemRole)
        ...
        private static bool CanViewWorkspaceResults(string role, Rule19WorkspaceStateViewModel? workspace)
        {
            if (workspace == null)
                return false;
        ...
        private async Task<Rule19ValidationSummary> ResolveExportSummaryAsync(Rule19ValidationSummary summary)
        {
            ...
            {
                var review = await _rule19.GetSavedRunAsync(savedRunId, user?.Email, includeFullResults: true);
            ...
            {
                var workspace = await _rule19.GetCurrentWorkspaceStateAsync(summary.ClientId, user?.Email, includeSummary: false);
                if (workspace?.RunId is int workspaceRunId && workspaceRunId > 0)
                {
                    var review = await _rule19.GetSavedRunAsync(workspaceRunId, user?.Email, includeFullResults: true);
                    if (review?.Summary != null)
                        return review.Summary;
                }
            }
        ...
        private async Task<object> RequireDataAnalystAsync<T>(Func<Task<T>> action)
        {
            ...
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            {
                return new { success = false, error = "Only the assigned data analyst can configure or run Rule 19." };
            }
