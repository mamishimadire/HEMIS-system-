...
{
    [Authorize]
    public class Rule53Controller : Controller
    {
        private readonly IRule53Service _rule53;
        private readonly IExportService _export;
        private readonly IAuditLogService _audit;
        private readonly UserManager<ApplicationUser> _users;
        private readonly ISystemDatabaseService _systemDb;

        public Rule53Controller(IRule53Service rule53, IExportService export, IAuditLogService audit, UserManager<ApplicationUser> users, ISystemDatabaseService systemDb)
        {
            _rule53 = rule53; _export = export; _audit = audit; _users = users; _systemDb = systemDb;
        }
        ...
        public async Task<IActionResult> Index(int clientId = 0)
        {
            ...
            ViewBag.ModuleNavigation = ModuleSequenceNavigationHelper.BuildForWorkspace(53, clientId);
            return View();
        }
        ...
        public async Task<IActionResult> GetWorkspaceState(int clientId)
        {
            ...
            var workspace = await _rule53.GetCurrentWorkspaceStateAsync(clientId, user?.Email);
        ...
        public async Task<IActionResult> Run(int id)
        {
            ...
            var review = await _rule53.GetSavedRunAsync(id, user?.Email);
            ...
            ViewBag.ModuleNavigation = ModuleSequenceNavigationHelper.BuildForSavedRun(53, review.ClientId, clientDetail?.ValidationRuns, role, review.CurrentUserEngagementRole);
        ...
        [HttpPost]
        public async Task<IActionResult> RunValidation([FromBody] Rule53ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);

            if (request.ClientId <= 0) return Json(new Rule53ValidationSummary { Success = false, Error = "Select an approved engagement before running validation." });
            if (!await _systemDb.CanAccessClientResultsAsync(request.ClientId, user, role)) return Json(new Rule53ValidationSummary { Success = false, Error = "You cannot access this engagement." });
            ...
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase) || !string.Equals(engagementRole, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
                return Json(new Rule53ValidationSummary { Success = false, Error = "Only the assigned data analyst can run Rule 53." });

            async Task<Rule53ValidationSummary> Execute(IRule53Service svc, IAuditLogService auditSvc)
            {
                var result = await svc.RunValidationAsync(request, user?.Email, user?.FullName ?? user?.Email);
                if (result.Success)
                    await auditSvc.LogAsync("run_validation", $"Rule 53 on client {request.ClientId}: {result.Status} ({result.FailCount} fail rows).", user?.Id, user?.Email);
                return result;
            }
            ...
            if (ValidationOperationHttpHelper.IsAsyncRequested(Request))
            {
                return ValidationOperationHttpHelper.Queue(this, HttpContext.RequestServices.GetRequiredService<IValidationOperationService>(),
                    ValidationOperationHttpHelper.ResolveOwnerKey(User), "Rule 53 validation",
                    async (sp, ct) => await Execute(sp.GetRequiredService<IRule53Service>(), sp.GetRequiredService<IAuditLogService>()));
            }

            return Json(await Execute(_rule53, _audit));
        }

        [HttpPost]
        public async Task<IActionResult> BeginWorkspaceEdit([FromBody] Rule53ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            if (!await CanEditWorkspaceAsync(request.ClientId, user, role)) return Json(new Rule53WorkspaceSaveResult { Success = false, Error = "Only the assigned data analyst can edit a saved workspace." });
            if (!request.RunId.HasValue || request.RunId.Value <= 0) return Json(new Rule53WorkspaceSaveResult { Success = false, Error = "Select a saved run before editing." });

            var result = await _rule53.BeginWorkspaceEditAsync(request.RunId.Value, user!.Email!, user.FullName);
            if (result.Success) await _audit.LogAsync("workspace_edit_started", $"DataAnalyst started editing Rule 53 run {request.RunId.Value}.", user?.Id, user?.Email);
            return Json(result);
        }

        [HttpPost]
        public async Task<IActionResult> SaveWorkspace([FromBody] Rule53ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            if (!await CanEditWorkspaceAsync(request.ClientId, user, role)) return Json(new Rule53WorkspaceSaveResult { Success = false, Error = "Only the assigned data analyst can save a workspace." });

            var result = await _rule53.SaveWorkspaceAsync(request, user!.Email!, user.FullName);
            if (result.Success) await _audit.LogAsync("save_validation_workspace", $"DataAnalyst saved Rule 53 workspace for client {request.ClientId}. Run: {result.Workspace?.RunId}", user?.Id, user?.Email);
            return Json(result);
        }

        [HttpPost]
        public async Task<IActionResult> SignOffWorkspace([FromBody] Rule53WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            var review = await _rule53.GetSavedRunAsync(model.RunId.Value, user?.Email);
            ...
            try
            {
                await _rule53.AddOrUpdateSignoffAsync(model.RunId.Value, user!.Email!, model.Comment);
                await _audit.LogAsync("signoff_validation_run", $"Rule 53 signoff saved for run {model.RunId.Value}", user.Id, user.Email);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule53WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
        ...
        [HttpPost]
        public async Task<IActionResult> GenerateSql([FromBody] Rule53ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
        ...
        public async Task<IActionResult> GenerateRScript([FromBody] Rule53ValidationRequest request)
        {
            ...
            return Json(await RequireDataAnalystAsync(async () => new Rule53SqlResult
            {
                Success = true,
                Sql = Rule53RScriptGenerator.Generate(request) + RScriptScaffold.BuildAutoExportFooter("Rule53")
            }));
        }

        [HttpPost, ValidateAntiForgeryToken]
        public async Task<IActionResult> AddSignoff(Rule53RunSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule53.GetSavedRunAsync(model.RunId, user?.Email);
            if (review == null) return NotFound();
            ...
            await _rule53.AddOrUpdateSignoffAsync(model.RunId, user!.Email!, model.Comment);
            await _audit.LogAsync("signoff_validation_run", $"{review.CurrentUserEngagementRole} signed off Rule 53 run {model.RunId}", user.Id, user.Email);
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            ...
            await _rule53.RemoveSignoffAsync(runId, user!.Email!);
            await _audit.LogAsync("remove_validation_signoff", $"{review.CurrentUserEngagementRole} removed signoff for Rule 53 run {runId}", user?.Id, user?.Email);
        ...
        [HttpPost]
        public async Task<IActionResult> DownloadSql([FromBody] Rule53ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
        ...
        private static bool CanViewSavedRun(Rule53RunReviewViewModel review, string systemRole)
            => ValidationRunAccessPolicy.CanViewSignedResults(systemRole, review.CurrentUserEngagementRole, review.HasDataAnalystSignoff);

        private static bool CanViewWorkspaceResults(string role, Rule53WorkspaceStateViewModel? workspace)
        {
        ...
        private async Task<object> RequireDataAnalystAsync<T>(Func<Task<T>> action) where T : class
        {
            ...
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
                return new { success = false, error = "Only the assigned data analyst can configure or run Rule 53." };
