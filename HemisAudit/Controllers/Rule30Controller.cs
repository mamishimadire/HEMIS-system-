...
namespace HemisAudit.Controllers
{
    ...
    public class Rule30Controller : Controller
    {
        ...
        public async Task<IActionResult> RunValidation([FromBody] Rule32ValidationRequest request)
        {
            ...
            async Task<Rule32ValidationSummary> ExecuteValidationAsync(IRule30Service ruleService, IAuditLogService auditService)
            {
            ...
            if (ValidationOperationHttpHelper.IsAsyncRequested(Request))
            {
                return ValidationOperationHttpHelper.Queue(
                    ...
                    async (sp, ct) => await ExecuteValidationAsync(
                        sp.GetRequiredService<IRule30Service>(),
                        sp.GetRequiredService<IAuditLogService>()));
            }

            var result = await ExecuteValidationAsync(_rule30, _audit);
            return Json(result);
        }
        ...
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule32WorkspaceSignoffInputModel model)
        {
            ...
            await _audit.LogAsync(
                ...
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 30 run {model.RunId.Value} from module workspace. Historical snapshot preserved; new current run {reopenedRunId.Value} created for continued review."
        ...
        public async Task<IActionResult> GenerateRScript([FromBody] Rule32ValidationRequest request)
        {
            ...
            {
                Success = true,
                Sql = Rule30RScriptGenerator.Generate(request) + RScriptScaffold.BuildAutoExportFooter("Rule30")
            }));
        }
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            ...
            var workspace = await _rule30.GetCurrentWorkspaceStateAsync(review.ClientId, user?.Email, includeSummary: false);
            var redirectRunId = workspace?.RunId ?? runId;
            var preservedHistory = workspace?.RunId.HasValue == true && workspace.RunId.Value != runId;
            await _audit.LogAsync(
                "remove_validation_signoff",
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 30 run {runId}. Historical snapshot preserved; new current run {redirectRunId} created for continued review."
