    ...
    public class Rule26Controller : Controller
    {
        ...
        public async Task<IActionResult> Run(int id)
        {
            ...
            try
            {
                review.GeneratedSql = await _rule26.GenerateSqlAsync(new Rule26ValidationRequest
                {
                    ...
                    BlankPayrollGroupPassCodes = review.Summary.BlankPayrollGroupPassCodes,
        ...
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule26WorkspaceSignoffInputModel model)
        {
            ...
            await _audit.LogAsync(
                ...
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 26 run {model.RunId.Value} from module workspace. Historical snapshot preserved; new current run {reopenedRunId.Value} created for continued review."
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            ...
            var workspace = await _rule26.GetCurrentWorkspaceStateAsync(review.ClientId, user?.Email, includeSummary: false);
            var redirectRunId = workspace?.RunId ?? runId;
            var preservedHistory = workspace?.RunId.HasValue == true && workspace.RunId.Value != runId;
            await _audit.LogAsync(
                "remove_validation_signoff",
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 26 run {runId}. Historical snapshot preserved; new current run {redirectRunId} created for continued review."
        ...
        private async Task<Rule26ValidationSummary> ResolveExportSummaryAsync(Rule26ValidationSummary summary)
        {
            ...
            {
                var workspace = await _rule26.GetCurrentWorkspaceStateAsync(summary.ClientId, user?.Email, includeSummary: false);
                if (workspace?.RunId is int workspaceRunId && workspaceRunId > 0)
                {
                    var review = await _rule26.GetSavedRunAsync(workspaceRunId, user?.Email);
                    if (review?.Summary != null)
                        return review.Summary;
                }
            }
