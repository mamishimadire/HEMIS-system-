...
namespace HemisAudit.Controllers
{
    ...
    public class Rule23Controller : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> GenerateRScript([FromBody] Rule23ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);

            if (request.ClientId > 0 && !await _systemDb.CanAccessClientResultsAsync(request.ClientId, user, role))
                return Json(new Rule23SqlResult { Success = false, Error = "You cannot access this engagement." });

            return Json(await RequireDataAnalystAsync(async () => new Rule23SqlResult
            {
                Success = true,
                Sql = Rule23RScriptGenerator.Generate(request) + RScriptScaffold.BuildAutoExportFooter("Rule23")
            }));
        }
        ...
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule23WorkspaceSignoffInputModel model)
        {
            ...
            await _audit.LogAsync(
                ...
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 23 run {model.RunId.Value} from module workspace. Historical snapshot preserved; new current run {reopenedRunId.Value} created for continued review."
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            ...
            var workspace = await _rule23.GetCurrentWorkspaceStateAsync(review.ClientId, user?.Email, includeSummary: false);
            var redirectRunId = workspace?.RunId ?? runId;
            var preservedHistory = workspace?.RunId.HasValue == true && workspace.RunId.Value != runId;
            await _audit.LogAsync(
                "remove_validation_signoff",
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 23 run {runId}. Historical snapshot preserved; new current run {redirectRunId} created for continued review."
        ...
        private async Task<Rule23ValidationSummary> ResolveExportSummaryAsync(Rule23ValidationSummary summary)
        {
            ...
            {
                var workspace = await _rule23.GetCurrentWorkspaceStateAsync(summary.ClientId, user?.Email, includeSummary: false);
                if (workspace?.RunId is int workspaceRunId && workspaceRunId > 0)
                {
                    var review = await _rule23.GetSavedRunAsync(workspaceRunId, user?.Email);
                    if (review?.Summary != null)
                        return review.Summary;
                }
            }
