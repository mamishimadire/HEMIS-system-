...
{
    [Authorize]
    public class Rule24Controller : Controller
    {
        private readonly IRule24Service _rule24;
        private readonly IExportService _export;
        private readonly IAuditLogService _audit;
        private readonly UserManager<ApplicationUser> _users;
        private readonly ISystemDatabaseService _systemDb;

        public Rule24Controller(
            IRule24Service rule24,
        ...
        public async Task<IActionResult> Index(int clientId = 0)
        {
            ...
            ViewBag.ModuleNavigation = ModuleSequenceNavigationHelper.BuildForWorkspace(24, clientId);
            return View();
        }
        ...
        public async Task<IActionResult> GetWorkspaceState(int clientId)
        {
            ...
            var workspace = await _rule24.GetCurrentWorkspaceStateAsync(clientId, user?.Email);
        ...
        public async Task<IActionResult> Run(int id)
        {
            ...
            var review = await _rule24.GetSavedRunAsync(id, user?.Email);
            if (review == null)
                return NotFound();
            ...
            review.GeneratedSql = await _rule24.GenerateSqlAsync(new Rule24ValidationRequest
            {
                ...
                ApprovalStatusColumn = review.Summary.ApprovalStatusColumn,
                ExcludedApprovalStatusValue = review.Summary.ExcludedApprovalStatusValue,
                Control1OnlyMode = review.Summary.Control1OnlyMode,
                AuditQualCodeColumn = review.Summary.AuditQualCodeColumn,
                H16QualCodeColumn = review.Summary.H16QualCodeColumn
            });
        ...
        public async Task<IActionResult> VerifyTables([FromBody] Rule24VerifyRequest request) =>
            Json(await RequireDataAnalystAsync(async () => await _rule24.VerifyTablesAsync(request)));

        [HttpPost]
        public async Task<IActionResult> RunValidation([FromBody] Rule24ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                return Json(new Rule24ValidationSummary
                {
                    Success = false,
                ...
                !string.Equals(engagementRole, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    Success = false,
                    Error = "Only the assigned data analyst can run Rule 24."
                });
            }

            async Task<Rule24ValidationSummary> ExecuteValidationAsync(IRule24Service ruleService, IAuditLogService auditService)
            {
                ...
                if (result.Success)
                {
                    await auditService.LogAsync(
                        "run_validation",
                        $"Rule 24 on client {request.ClientId}: {result.Status} ({result.FailCount} mismatches), run {result.SavedRunId}",
                        user?.Id,
                        user?.Email);
                }
            ...
            if (ValidationOperationHttpHelper.IsAsyncRequested(Request))
            {
                return ValidationOperationHttpHelper.Queue(
                    ...
                    async (sp, ct) => await ExecuteValidationAsync(
                        sp.GetRequiredService<IRule24Service>(),
                        sp.GetRequiredService<IAuditLogService>()));
            }

            var result = await ExecuteValidationAsync(_rule24, _audit);
            return Json(result);
        }

        [HttpPost]
        public async Task<IActionResult> BeginWorkspaceEdit([FromBody] Rule24ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                return Json(new Rule24WorkspaceSaveResult
                {
                    Success = false,
            ...
            var result = await _rule24.BeginWorkspaceEditAsync(request.RunId.Value, user!.Email!, user.FullName);
            if (result.Success)
            {
                await _audit.LogAsync(
                    "workspace_edit_started",
                    $"DataAnalyst started editing Rule 24 run {request.RunId.Value}. Existing signoffs were cleared.",
                    user?.Id,
                    user?.Email);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> SaveWorkspace([FromBody] Rule24ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            var result = await _rule24.SaveWorkspaceAsync(request, user!.Email!, user.FullName);
            if (result.Success)
            {
                await _audit.LogAsync(
                    "save_validation_workspace",
                    $"DataAnalyst saved Rule 24 workspace for client {request.ClientId}. Signoffs cleared: {result.ClearedSignoffCount ?? 0}",
                    user?.Id,
                    user?.Email);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> GenerateSql([FromBody] Rule24ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                return Json(new Rule24SqlResult
                {
                    Success = false,
                    Error = "You cannot access this engagement."
                });
            ...
            return Json(await RequireDataAnalystAsync(async () =>
                ...
                {
                    Success = true,
                    Sql = await _rule24.GenerateSqlAsync(request)
                }));
        }
        [HttpPost]
        public async Task<IActionResult> GenerateRScript([FromBody] Rule24ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);

            if (request.ClientId > 0 && !await _systemDb.CanAccessClientResultsAsync(request.ClientId, user, role))
                return Json(new Rule24SqlResult { Success = false, Error = "You cannot access this engagement." });

            return Json(await RequireDataAnalystAsync(async () => new Rule24SqlResult
            {
                Success = true,
                Sql = Rule24RScriptGenerator.Generate(request) + RScriptScaffold.BuildAutoExportFooter("Rule24")
            }));
        }

        [HttpPost]
        public async Task<IActionResult> SignOffWorkspace([FromBody] Rule24WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            var review = await _rule24.GetSavedRunAsync(model.RunId.Value, user?.Email);
            ...
            try
            {
                await _rule24.AddOrUpdateSignoffAsync(model.RunId.Value, user!.Email!, model.Comment);
                await _audit.LogAsync(
                    "signoff_validation_run",
                    $"Rule 24 signoff saved for run {model.RunId.Value} from module workspace",
                    user.Id,
                    user.Email);
            }
            catch (Exception ex)
        ...
        [HttpPost]
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule24WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
        ...
        public async Task<IActionResult> AddSignoff(Rule24RunSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule24.GetSavedRunAsync(model.RunId, user?.Email);
            if (review == null)
                return NotFound();
            ...
            await _rule24.AddOrUpdateSignoffAsync(model.RunId, user!.Email!, model.Comment);
            await _audit.LogAsync(
                "signoff_validation_run",
                $"{review.CurrentUserEngagementRole} signed off Rule 24 run {model.RunId}",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule24.GetSavedRunAsync(runId, user?.Email);
            if (review == null)
                return NotFound();
        ...
        public async Task<IActionResult> DownloadSavedExcel(int runId)
        {
            ...
            return File(
                ...
                $"Rule24_Qualification_Reconciliation_Run_{runId}.xlsx");
        }
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            ...
            return File(bytes, "text/csv", $"Rule24_Fail_Exceptions_Run_{runId}.csv");
        }
        ...
        public async Task<IActionResult> DownloadSavedSql(int runId)
        {
            ...
            var bytes = _export.ExportSql(await _rule24.GenerateSqlAsync(request));
            return File(bytes, "application/sql", $"Rule24_Qualification_Reconciliation_{runId}.sql");
        }

        [HttpPost]
        public async Task<IActionResult> DownloadExcel([FromBody] Rule24ValidationSummary summary)
        {
            summary = await ResolveExportSummaryAsync(summary);
            var fileName = $"Rule24_Qualification_Reconciliation_{Ts()}.xlsx";
        ...
        [HttpPost]
        public async Task<IActionResult> DownloadCsv([FromBody] Rule24ValidationSummary summary)
        {
            summary = await ResolveExportSummaryAsync(summary);
            var fileName = $"Rule24_Fail_Exceptions_{Ts()}.csv";
            var bytes = _export.ExportCsv(summary);
            return File(bytes, "text/csv", fileName);
        }

        [HttpPost]
        public async Task<IActionResult> DownloadSql([FromBody] Rule24ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            ...
            var fileName = $"Rule24_Qualification_Reconciliation_{Ts()}.sql";
            var bytes = _export.ExportSql(await _rule24.GenerateSqlAsync(request));
            return File(bytes, "application/sql", fileName);
        }

        private async Task<Rule24RunReviewViewModel?> LoadAuthorizedSavedRunAsync(int runId, bool requireDownloadAccess)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule24.GetSavedRunAsync(runId, user?.Email);
        ...
        private static bool CanDownloadSavedRun(Rule24RunReviewViewModel review, string systemRole)
        ...
        private static bool CanViewSavedRun(Rule24RunReviewViewModel review, string systemRole)
        ...
        private static bool CanViewWorkspaceResults(string role, Rule24WorkspaceStateViewModel? workspace)
        {
            if (workspace == null)
                return false;
        ...
        private async Task<Rule24ValidationSummary> ResolveExportSummaryAsync(Rule24ValidationSummary summary)
        {
            var user = await _users.GetUserAsync(User);

            if (summary.SavedRunId is int savedRunId && savedRunId > 0)
            {
                var review = await _rule24.GetSavedRunAsync(savedRunId, user?.Email);
            ...
            {
                var workspace = await _rule24.GetCurrentWorkspaceStateAsync(summary.ClientId, user?.Email, includeSummary: false);
                if (workspace?.RunId is int workspaceRunId && workspaceRunId > 0)
                {
                    var review = await _rule24.GetSavedRunAsync(workspaceRunId, user?.Email);
                    if (review?.Summary != null)
                        return review.Summary;
                }
            }
        ...
        private async Task<object> RequireDataAnalystAsync<T>(Func<Task<T>> action)
        {
            ...
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            {
                return new { success = false, error = "Only the assigned data analyst can configure or run Rule 24." };
            }
