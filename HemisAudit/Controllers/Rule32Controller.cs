...
{
    [Authorize]
    public class Rule32Controller : Controller
    {
        private readonly IRule32Service _rule32;
        private readonly IExportService _export;
        private readonly IAuditLogService _audit;
        private readonly UserManager<ApplicationUser> _users;
        private readonly ISystemDatabaseService _systemDb;

        public Rule32Controller(
            IRule32Service rule32,
        ...
        public async Task<IActionResult> Index(int clientId = 0)
        {
            ...
            ViewBag.ModuleNavigation = ModuleSequenceNavigationHelper.BuildForWorkspace(32, clientId);
            return View();
        }
        ...
        public async Task<IActionResult> GetWorkspaceState(int clientId)
        {
            ...
            var workspace = await _rule32.GetCurrentWorkspaceStateAsync(clientId, user?.Email);
        ...
        public async Task<IActionResult> Run(int id)
        {
            ...
            var review = await _rule32.GetSavedRunAsync(id, user?.Email);
            if (review == null)
                return NotFound();
        ...
        public async Task<IActionResult> GetDatabases([FromBody] ConnectionViewModel model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule32.GetDatabasesAsync(model.Server, model.Driver)));

        [HttpPost]
        public async Task<IActionResult> GetTables([FromBody] ConnectionViewModel model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule32.GetTablesAsync(model.Server, model.Database, model.Driver)));

        [HttpPost]
        public async Task<IActionResult> GetColumns([FromBody] Rule32GetColumnsRequest model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule32.GetColumnsAsync(model.Server, model.Database, model.Driver, model.TableName)));
        ...
        public async Task<IActionResult> LoadFilterValues([FromBody] Rule32FilterValueRequest model) =>
            Json(await RequireDataAnalystAsync(async () =>
                await _rule32.GetFilterValuesAsync(model.Server, model.Database, model.Driver, model.TableName, model.ErrorTypeColumn)));

        [HttpPost]
        public async Task<IActionResult> VerifyTable([FromBody] Rule32VerifyRequest request) =>
            Json(await RequireDataAnalystAsync(async () => await _rule32.VerifyTableAsync(request)));
        ...
        public async Task<IActionResult> RunValidation([FromBody] Rule32ValidationRequest request)
        {
                ...
                !string.Equals(engagementRole, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    Success = false,
                    Error = "Only the assigned data analyst can run Rule 32."
                });
            }

            async Task<Rule32ValidationSummary> ExecuteValidationAsync(IRule32Service ruleService, IAuditLogService auditService)
            {
                ...
                if (result.Success)
                {
                    await auditService.LogAsync(
                        "run_validation",
                        $"Rule 32 on client {request.ClientId}: {result.Status} ({result.FailCount} remaining), run {result.SavedRunId}",
                        user?.Id,
                        user?.Email);
                }
            ...
            if (ValidationOperationHttpHelper.IsAsyncRequested(Request))
            {
                return ValidationOperationHttpHelper.Queue(
                    ...
                    async (sp, ct) => await ExecuteValidationAsync(
                        sp.GetRequiredService<IRule32Service>(),
                        sp.GetRequiredService<IAuditLogService>()));
            }

            var result = await ExecuteValidationAsync(_rule32, _audit);
            return Json(result);
        }
        ...
        public async Task<IActionResult> BeginWorkspaceEdit([FromBody] Rule32ValidationRequest request)
        {
            ...
            var result = await _rule32.BeginWorkspaceEditAsync(request.RunId.Value, user!.Email!, user.FullName);
            if (result.Success)
            {
                await _audit.LogAsync(
                    "workspace_edit_started",
                    $"DataAnalyst started editing Rule 32 run {request.RunId.Value}. Existing signoffs were cleared.",
                    user?.Id,
                    user?.Email);
            }
        ...
        public async Task<IActionResult> SaveWorkspace([FromBody] Rule32ValidationRequest request)
        {
            ...
            var result = await _rule32.SaveWorkspaceAsync(request, user!.Email!, user.FullName);
            if (result.Success)
            {
                await _audit.LogAsync(
                    "save_validation_workspace",
                    $"DataAnalyst saved Rule 32 workspace for client {request.ClientId}. Signoffs cleared: {result.ClearedSignoffCount ?? 0}",
                    user?.Id,
                    user?.Email);
            }
        ...
        public async Task<IActionResult> SignOffWorkspace([FromBody] Rule32WorkspaceSignoffInputModel model)
        {
            ...
            var review = await _rule32.GetSavedRunAsync(model.RunId.Value, user?.Email);
            ...
            try
            {
                await _rule32.AddOrUpdateSignoffAsync(model.RunId.Value, user!.Email!, model.Comment);
                await _audit.LogAsync(
                    "signoff_validation_run",
                    $"Rule 32 signoff saved for run {model.RunId.Value} from module workspace",
                    user.Id,
                    user.Email);
            }
            catch (Exception ex)
            ...
            var workspace = await _rule32.GetCurrentWorkspaceStateAsync(model.ClientId, user?.Email, includeSummary: true);
            var resultsVisible = CanViewWorkspaceResults(role, workspace);
        ...
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule32WorkspaceSignoffInputModel model)
        {
            ...
            try
            {
                await _rule32.RemoveSignoffAsync(model.RunId.Value, user!.Email!);
            }
            catch (Exception ex)
            {
                return Json(new { success = false, error = ex.Message });
            ...
            await _audit.LogAsync(
                ...
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 32 run {model.RunId.Value} from module workspace. Historical snapshot preserved; new current run {reopenedRunId.Value} created for continued review."
                    : $"{review.CurrentUserEngagementRole} removed signoff for Rule 32 run {model.RunId.Value} from module workspace",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> GenerateSql([FromBody] Rule32ValidationRequest request)
        {
            ...
            return Json(await RequireDataAnalystAsync(async () =>
                ...
                {
                    Success = true,
                    Sql = await _rule32.GenerateSqlAsync(request)
                }));
        }
        [HttpPost]
        public async Task<IActionResult> GenerateRScript([FromBody] Rule32ValidationRequest request)
        {
            ...
            {
                Success = true,
                Sql = Rule32RScriptGenerator.Generate(request) + RScriptScaffold.BuildAutoExportFooter("Rule32")
            }));
        }
        ...
        public async Task<IActionResult> AddSignoff(Rule32RunSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule32.GetSavedRunAsync(model.RunId, user?.Email);
            if (review == null)
                return NotFound();
            ...
            await _rule32.AddOrUpdateSignoffAsync(model.RunId, user!.Email!, model.Comment);
            await _audit.LogAsync(
                "signoff_validation_run",
                $"{review.CurrentUserEngagementRole} signed off Rule 32 run {model.RunId}",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule32.GetSavedRunAsync(runId, user?.Email);
            if (review == null)
                return NotFound();
            ...
            await _rule32.RemoveSignoffAsync(runId, user!.Email!);
            var workspace = await _rule32.GetCurrentWorkspaceStateAsync(review.ClientId, user?.Email, includeSummary: false);
            var redirectRunId = workspace?.RunId ?? runId;
            var preservedHistory = workspace?.RunId.HasValue == true && workspace.RunId.Value != runId;
            await _audit.LogAsync(
                "remove_validation_signoff",
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 32 run {runId}. Historical snapshot preserved; new current run {redirectRunId} created for continued review."
                    : $"{review.CurrentUserEngagementRole} removed signoff for Rule 32 run {runId}",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> DownloadSavedExcel(int runId)
        {
            ...
            return File(bytes,
                ...
                $"Rule32_Fatal_Errors_Run_{runId}.xlsx");
        }
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            ...
            return File(bytes, "text/csv", $"Rule32_Excluded_and_Remaining_Run_{runId}.csv");
        }
        ...
        public async Task<IActionResult> DownloadSavedExceptionsCsv(int runId)
        {
            ...
            return File(bytes, "text/csv", $"Rule32_Remaining_Errors_Run_{runId}.csv");
        }
        ...
        public async Task<IActionResult> DownloadSavedSql(int runId)
        {
            ...
            var bytes = _export.ExportSql(await _rule32.GenerateSqlAsync(request));
            return File(bytes, "application/sql", $"Rule32_Fatal_Errors_{runId}.sql");
        }
        ...
        {
            summary = await ResolveExportSummaryAsync(summary);
            var fileName = $"Rule32_Excluded_and_Remaining_{Ts()}.csv";
        ...
        {
            summary = await ResolveExportSummaryAsync(summary);
            var fileName = $"Rule32_Remaining_Errors_{Ts()}.csv";
        ...
        public async Task<IActionResult> DownloadSql([FromBody] Rule32ValidationRequest request)
        {
            ...
            var fileName = $"Rule32_Fatal_Errors_{Ts()}.sql";
            var bytes = _export.ExportSql(await _rule32.GenerateSqlAsync(request));
        ...
        private async Task<Rule32ValidationSummary> ResolveExportSummaryAsync(Rule32ValidationSummary summary)
        {
            ...
            {
                var review = await _rule32.GetSavedRunAsync(savedRunId, user?.Email);
            ...
            {
                var workspace = await _rule32.GetCurrentWorkspaceStateAsync(summary.ClientId, user?.Email, includeSummary: false);
                if (workspace?.RunId is int workspaceRunId && workspaceRunId > 0)
                {
                    var review = await _rule32.GetSavedRunAsync(workspaceRunId, user?.Email);
                    if (review?.Summary != null)
                        return review.Summary;
                }
            }
        ...
        private async Task<object> RequireDataAnalystAsync<T>(Func<Task<T>> action)
        {
            ...
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            {
                return new { success = false, error = "Only the assigned data analyst can configure or run Rule 32." };
            }
