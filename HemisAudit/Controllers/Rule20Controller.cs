...
{
    [Authorize]
    public class Rule20Controller : Controller
    {
        private readonly IRule20Service _rule20;
        private readonly IExportService _export;
        private readonly IAuditLogService _audit;
        private readonly UserManager<ApplicationUser> _users;
        private readonly ISystemDatabaseService _systemDb;
        private readonly ILogger<Rule20Controller> _logger;

        public Rule20Controller(
            IRule20Service rule20,
            IExportService export,
            IAuditLogService audit,
            UserManager<ApplicationUser> users,
            ISystemDatabaseService systemDb,
            ILogger<Rule20Controller> logger)
        {
            _rule20 = rule20;
            _export = export;
            _audit = audit;
        ...
        public async Task<IActionResult> Index(int clientId = 0)
        {
            ...
            ViewBag.ModuleNavigation = ModuleSequenceNavigationHelper.BuildForWorkspace(20, clientId);
            return View();
        }
        ...
        public async Task<IActionResult> GetWorkspaceState(int clientId)
        {
            ...
            var workspace = await _rule20.GetCurrentWorkspaceStateAsync(clientId, user?.Email);
        ...
        public async Task<IActionResult> Run(int id)
        {
            ...
            var review = await _rule20.GetSavedRunAsync(id, user?.Email);
            if (review == null)
                return NotFound();
            ...
            review.GeneratedSql = await _rule20.GenerateSqlAsync(new Rule20ValidationRequest
            {
        ...
        public async Task<IActionResult> GetDatabases([FromBody] ConnectionViewModel model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule20.GetDatabasesAsync(model.Server, model.Driver)));
        ...
        public async Task<IActionResult> VerifyTables([FromBody] Rule20VerifyRequest request) =>
            Json(await RequireDataAnalystAsync(async () => await _rule20.VerifyTablesAsync(request)));

        [HttpPost]
        public async Task<IActionResult> RunValidation([FromBody] Rule20ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            _logger.LogInformation(
                "Rule20 RunValidation requested by {Email}. ClientId={ClientId}, Server={Server}, Database={Database}, StudTable={StudTable}, QualTable={QualTable}, CregTable={CregTable}, CrseTable={CrseTable}",
                user?.Email,
                request.ClientId,
                request.Server,
                request.Database,
            ...
            {
                _logger.LogWarning("Rule20 rejected because no client was selected.");
                return Json(new Rule20ValidationSummary
                {
                    Success = false,
            ...
            {
                _logger.LogWarning("Rule20 rejected for {Email} because the user cannot access client {ClientId}.", user?.Email, request.ClientId);
                return Json(new Rule20ValidationSummary
                {
                    Success = false,
            ...
            {
                _logger.LogWarning(
                    "Rule20 rejected for {Email}. SystemRole={SystemRole}, EngagementRole={EngagementRole}, ClientId={ClientId}",
                    user?.Email,
                    role,
                    engagementRole,
                    request.ClientId);
                ...
                {
                    Success = false,
                    Error = "Only the assigned data analyst can run Rule 20."
                });
            }

            async Task<Rule20ValidationSummary> ExecuteValidationAsync(IRule20Service ruleService, IAuditLogService auditService)
            {
                var result = await ruleService.RunValidationAsync(request, user?.Email, user?.FullName ?? user?.Email);
                _logger.LogInformation(
                    "Rule20 RunValidation completed for {Email}. Success={Success}, Status={Status}, Error={Error}, TotalValidated={TotalValidated}, SavedRunId={SavedRunId}",
                    user?.Email,
                    result.Success,
                    result.Status,
                    result.Error,
                ...
                if (result.Success)
                {
                    await auditService.LogAsync(
                        "run_validation",
                        $"Rule 20 on client {request.ClientId}: {result.Status} ({result.FailCount} fail rows), run {result.SavedRunId}",
                        user?.Id,
                        user?.Email);
                }
            ...
            if (ValidationOperationHttpHelper.IsAsyncRequested(Request))
            {
                return ValidationOperationHttpHelper.Queue(
                    ...
                    async (sp, ct) => await ExecuteValidationAsync(
                        sp.GetRequiredService<IRule20Service>(),
                        sp.GetRequiredService<IAuditLogService>()));
            }

            var result = await ExecuteValidationAsync(_rule20, _audit);
            return Json(result);
        }

        [HttpPost]
        public async Task<IActionResult> BeginWorkspaceEdit([FromBody] Rule20ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                return Json(new Rule20WorkspaceSaveResult
                {
                    Success = false,
            ...
            var result = await _rule20.BeginWorkspaceEditAsync(request.RunId.Value, user!.Email!, user.FullName);
            if (result.Success)
            {
                await _audit.LogAsync(
                    "workspace_edit_started",
                    $"DataAnalyst started editing Rule 20 run {request.RunId.Value}. Existing signoffs were cleared.",
                    user?.Id,
                    user?.Email);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> SaveWorkspace([FromBody] Rule20ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            var result = await _rule20.SaveWorkspaceAsync(request, user!.Email!, user.FullName);
            if (result.Success)
            {
                await _audit.LogAsync(
                    "save_validation_workspace",
                    $"DataAnalyst saved Rule 20 workspace for client {request.ClientId}. Signoffs cleared: {result.ClearedSignoffCount ?? 0}",
                    user?.Id,
                    user?.Email);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> GenerateSql([FromBody] Rule20ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                return Json(new Rule20SqlResult
                {
                    Success = false,
                    Error = "You cannot access this engagement."
                });
            ...
            return Json(await RequireDataAnalystAsync(async () =>
                ...
                {
                    Success = true,
                    Sql = await _rule20.GenerateSqlAsync(request)
                }));
        }
        [HttpPost]
        public async Task<IActionResult> GenerateRScript([FromBody] Rule20ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);

            if (request.ClientId > 0 && !await _systemDb.CanAccessClientResultsAsync(request.ClientId, user, role))
                return Json(new Rule20SqlResult { Success = false, Error = "You cannot access this engagement." });

            return Json(await RequireDataAnalystAsync(async () => new Rule20SqlResult
            {
                Success = true,
                Sql = Rule20RScriptGenerator.Generate(request) + RScriptScaffold.BuildAutoExportFooter("Rule20")
            }));
        }

        [HttpPost]
        public async Task<IActionResult> SignOffWorkspace([FromBody] Rule20WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            var review = await _rule20.GetSavedRunAsync(model.RunId.Value, user?.Email);
            ...
            try
            {
                await _rule20.AddOrUpdateSignoffAsync(model.RunId.Value, user!.Email!, model.Comment);
                await _audit.LogAsync(
                    "signoff_validation_run",
                    $"Rule 20 signoff saved for run {model.RunId.Value} from module workspace",
                    user.Id,
                    user.Email);
            }
            catch (Exception ex)
            ...
            var workspace = await _rule20.GetCurrentWorkspaceStateAsync(model.ClientId, user?.Email, includeSummary: false);
            var resultsVisible = CanViewWorkspaceResults(role, workspace);
        ...
        [HttpPost]
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule20WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            try
            {
                await _rule20.RemoveSignoffAsync(model.RunId.Value, user!.Email!);
            }
            catch (Exception ex)
            {
                return Json(new { success = false, error = ex.Message });
            ...
            await _audit.LogAsync(
                ...
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 20 run {model.RunId.Value} from module workspace. Historical snapshot preserved; new current run {reopenedRunId.Value} created for continued review."
                    : $"{review.CurrentUserEngagementRole} removed signoff for Rule 20 run {model.RunId.Value} from module workspace",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> AddSignoff(Rule20RunSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule20.GetSavedRunAsync(model.RunId, user?.Email);
            if (review == null)
                return NotFound();
            ...
            await _rule20.AddOrUpdateSignoffAsync(model.RunId, user!.Email!, model.Comment);
            await _audit.LogAsync(
                "signoff_validation_run",
                $"{review.CurrentUserEngagementRole} signed off Rule 20 run {model.RunId}",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule20.GetSavedRunAsync(runId, user?.Email);
            if (review == null)
                return NotFound();
            ...
            await _rule20.RemoveSignoffAsync(runId, user!.Email!);
            var workspace = await _rule20.GetCurrentWorkspaceStateAsync(review.ClientId, user?.Email, includeSummary: false);
            var redirectRunId = workspace?.RunId ?? runId;
            var preservedHistory = workspace?.RunId.HasValue == true && workspace.RunId.Value != runId;
            await _audit.LogAsync(
                "remove_validation_signoff",
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 20 run {runId}. Historical snapshot preserved; new current run {redirectRunId} created for continued review."
                    : $"{review.CurrentUserEngagementRole} removed signoff for Rule 20 run {runId}",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> DownloadSavedExcel(int runId)
        {
            ...
            return File(bytes, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", $"Rule20_Foundation_Validation_Run_{runId}.xlsx");
        }
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            ...
            return File(bytes, "text/csv", $"Rule20_Foundation_Validation_Run_{runId}.csv");
        }
        ...
        public async Task<IActionResult> DownloadSavedSql(int runId)
        {
            ...
            var request = new Rule20ValidationRequest
            {
                ClientId = review.ClientId,
                Server = review.SourceServer,
                Database = review.Summary.Database,
        ...
        [HttpPost]
        public async Task<IActionResult> DownloadSql([FromBody] Rule20ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            ...
            var fileName = $"Rule20_Foundation_Validation_{Ts()}.sql";
            var bytes = _export.ExportSql(await _rule20.GenerateSqlAsync(request));
            return File(bytes, "application/sql", fileName);
        }

        private async Task<Rule20RunReviewViewModel?> LoadAuthorizedSavedRunAsync(int runId, bool requireDownloadAccess)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule20.GetSavedRunAsync(runId, user?.Email, includeFullResults: requireDownloadAccess);
            if (review == null)
            {
        ...
        private static bool CanDownloadSavedRun(Rule20RunReviewViewModel review, string systemRole)
        ...
        private static bool CanViewSavedRun(Rule20RunReviewViewModel review, string systemRole)
        ...
        private static bool CanViewWorkspaceResults(string role, Rule20WorkspaceStateViewModel? workspace)
        {
            if (workspace == null)
                return false;
        ...
        private async Task<Rule20ValidationSummary> ResolveExportSummaryAsync(Rule20ValidationSummary summary)
        {
            ...
            {
                var review = await _rule20.GetSavedRunAsync(savedRunId, user?.Email, includeFullResults: true);
            ...
            {
                var workspace = await _rule20.GetCurrentWorkspaceStateAsync(summary.ClientId, user?.Email, includeSummary: false);
                if (workspace?.RunId is int workspaceRunId && workspaceRunId > 0)
                {
                    var review = await _rule20.GetSavedRunAsync(workspaceRunId, user?.Email, includeFullResults: true);
                    if (review?.Summary != null)
                    {
                        return review.Summary;
                    }
        ...
        private async Task<object> RequireDataAnalystAsync<T>(Func<Task<T>> action)
        {
            ...
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            {
                return new { success = false, error = "Only the assigned data analyst can configure or run Rule 20." };
            }
