...
{
    [Authorize]
    public class Rule59Controller : Controller
    {
        private readonly IRule59Service _rule59;
        private readonly IExportService _export;
        private readonly IAuditLogService _audit;
        private readonly UserManager<ApplicationUser> _users;
        private readonly ISystemDatabaseService _systemDb;

        public Rule59Controller(IRule59Service rule59, IExportService export, IAuditLogService audit, UserManager<ApplicationUser> users, ISystemDatabaseService systemDb)
        {
            _rule59 = rule59; _export = export; _audit = audit; _users = users; _systemDb = systemDb;
        }
        ...
        public async Task<IActionResult> Index(int clientId = 0)
        {
            ...
            ViewBag.ModuleNavigation = ModuleSequenceNavigationHelper.BuildForWorkspace(59, clientId);
            return View();
        }
        ...
        public async Task<IActionResult> Run(int id)
        {
            ...
            var review = await _rule59.GetSavedRunAsync(id, user?.Email);
        ...
        public async Task<IActionResult> RunValidation([FromBody] Rule59ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            if (request.ClientId <= 0) return Json(new Rule59ValidationSummary { Success = false, Error = "Select an approved engagement before running validation." });
            if (!await _systemDb.CanAccessClientResultsAsync(request.ClientId, user, role)) return Json(new Rule59ValidationSummary { Success = false, Error = "You cannot access this engagement." });
            ...
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase) || !string.Equals(engagementRole, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
                return Json(new Rule59ValidationSummary { Success = false, Error = "Only the assigned data analyst can run Rule 59." });

            async Task<Rule59ValidationSummary> Execute(IRule59Service svc, IAuditLogService auditSvc)
            {
                var result = await svc.RunValidationAsync(request, user?.Email, user?.FullName ?? user?.Email);
                if (result.Success)
                    await auditSvc.LogAsync("run_validation", $"Rule 59 on client {request.ClientId}: {result.Status} ({result.FailCount} fail rows).", user?.Id, user?.Email);
                return result;
            }
            ...
            if (ValidationOperationHttpHelper.IsAsyncRequested(Request))
            {
                return ValidationOperationHttpHelper.Queue(this, HttpContext.RequestServices.GetRequiredService<IValidationOperationService>(),
                    ValidationOperationHttpHelper.ResolveOwnerKey(User), "Rule 59 validation",
                    async (sp, ct) => await Execute(sp.GetRequiredService<IRule59Service>(), sp.GetRequiredService<IAuditLogService>()));
            }

            return Json(await Execute(_rule59, _audit));
        }

        [HttpPost]
        public async Task<IActionResult> BeginWorkspaceEdit([FromBody] Rule59ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            if (!await CanEditWorkspaceAsync(request.ClientId, user, role)) return Json(new Rule59WorkspaceSaveResult { Success = false, Error = "Only the assigned data analyst can edit a saved workspace." });
            if (!request.RunId.HasValue || request.RunId.Value <= 0) return Json(new Rule59WorkspaceSaveResult { Success = false, Error = "Select a saved run before editing." });
            var result = await _rule59.BeginWorkspaceEditAsync(request.RunId.Value, user!.Email!, user.FullName);
            if (result.Success) await _audit.LogAsync("workspace_edit_started", $"DataAnalyst started editing Rule 59 run {request.RunId.Value}.", user?.Id, user?.Email);
            return Json(result);
        }

        [HttpPost]
        public async Task<IActionResult> SaveWorkspace([FromBody] Rule59ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            if (!await CanEditWorkspaceAsync(request.ClientId, user, role)) return Json(new Rule59WorkspaceSaveResult { Success = false, Error = "Only the assigned data analyst can save a workspace." });
            var result = await _rule59.SaveWorkspaceAsync(request, user!.Email!, user.FullName);
            if (result.Success) await _audit.LogAsync("save_validation_workspace", $"DataAnalyst saved Rule 59 workspace for client {request.ClientId}. Run: {result.Workspace?.RunId}", user?.Id, user?.Email);
            return Json(result);
        }

        [HttpPost]
        public async Task<IActionResult> SignOffWorkspace([FromBody] Rule59WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            var review = await _rule59.GetSavedRunAsync(model.RunId.Value, user?.Email);
            ...
            try
            {
                await _rule59.AddOrUpdateSignoffAsync(model.RunId.Value, user!.Email!, model.Comment);
                await _audit.LogAsync("signoff_validation_run", $"Rule 59 signoff saved for run {model.RunId.Value}", user.Id, user.Email);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule59WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
        ...
        [HttpPost]
        public async Task<IActionResult> GenerateSql([FromBody] Rule59ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
        ...
        public async Task<IActionResult> GenerateRScript([FromBody] Rule59ValidationRequest request)
        {
            ...
            return Json(await RequireDataAnalystAsync(async () => new Rule59SqlResult
            {
                Success = true,
                Sql = Rule59RScriptGenerator.Generate(request) + RScriptScaffold.BuildAutoExportFooter("Rule59")
            }));
        }

        [HttpPost, ValidateAntiForgeryToken]
        public async Task<IActionResult> AddSignoff(Rule59RunSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule59.GetSavedRunAsync(model.RunId, user?.Email);
            if (review == null) return NotFound();
            var clientDetail = await _systemDb.GetClientDetailAsync(review.ClientId, user, role);
            ...
            await _rule59.AddOrUpdateSignoffAsync(model.RunId, user!.Email!, model.Comment);
            await _audit.LogAsync("signoff_validation_run", $"{review.CurrentUserEngagementRole} signed off Rule 59 run {model.RunId}", user.Id, user.Email);
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            ...
            await _rule59.RemoveSignoffAsync(runId, user!.Email!);
            await _audit.LogAsync("remove_validation_signoff", $"{review.CurrentUserEngagementRole} removed signoff for Rule 59 run {runId}", user?.Id, user?.Email);
        ...
        [HttpPost]
        public async Task<IActionResult> DownloadExcel([FromBody] Rule59ValidationSummary summary)
        {
            var resolved = await ResolveExportSummaryAsync(summary);
            return File(_export.ExportRule59Excel(resolved), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", $"Rule59_SFTE_VALPAC_Production_{Ts()}.xlsx");
        }

        [HttpPost]
        public async Task<IActionResult> DownloadCsv([FromBody] Rule59ValidationSummary summary)
        {
            var resolved = await ResolveExportSummaryAsync(summary);
            return File(_export.ExportRule59Csv(resolved), "text/csv", $"Rule59_SFTE_VALPAC_Production_{Ts()}.csv");
        }

        [HttpPost]
        public async Task<IActionResult> DownloadSql([FromBody] Rule59ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
        ...
        private async Task<Rule59ValidationSummary> ResolveExportSummaryAsync(Rule59ValidationSummary summary)
        {
            var user = await _users.GetUserAsync(User);
        ...
        private static bool CanDownloadSavedRun(Rule59RunReviewViewModel review, string systemRole)
        ...
        private static bool CanViewSavedRun(Rule59RunReviewViewModel review, string systemRole)
            => ValidationRunAccessPolicy.CanViewSignedResults(systemRole, review.CurrentUserEngagementRole, review.HasDataAnalystSignoff);

        private static bool CanViewWorkspaceResults(string role, Rule59WorkspaceStateViewModel? workspace)
        {
        ...
        private async Task<object> RequireDataAnalystAsync<T>(Func<Task<T>> action) where T : class
        {
            ...
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
                return new { success = false, error = "Only the assigned data analyst can configure or run Rule 59." };
