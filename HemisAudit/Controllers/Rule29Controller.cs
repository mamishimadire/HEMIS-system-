...
{
    [Authorize]
    public class Rule29Controller : Controller
    {
        private readonly IRule29Service _rule29;
        private readonly IExportService _export;
        private readonly IAuditLogService _audit;
        private readonly UserManager<ApplicationUser> _users;
        private readonly ISystemDatabaseService _systemDb;

        public Rule29Controller(
            IRule29Service rule29,
        ...
        public async Task<IActionResult> Index(int clientId = 0)
        {
            ...
            ViewBag.ModuleNavigation = ModuleSequenceNavigationHelper.BuildForWorkspace(29, clientId);
            return View();
        }
        ...
        public async Task<IActionResult> GetWorkspaceState(int clientId)
        {
            ...
            var workspace = await _rule29.GetCurrentWorkspaceStateAsync(clientId, user?.Email);
        ...
        public async Task<IActionResult> Run(int id)
        {
            ...
            var review = await _rule29.GetSavedRunAsync(id, user?.Email);
            if (review == null)
                return NotFound();
            ...
            try
            {
                review.GeneratedSql = await _rule29.GenerateSqlAsync(new Rule29ValidationRequest
                {
                    ...
                    BreakdownColumn = review.Summary.BreakdownColumn,
                    SampleSize = review.Summary.SampleSize,
                    ShowAllRecords = review.Summary.ShowAllRecords
                });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetDatabases([FromBody] ConnectionViewModel model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule29.GetDatabasesAsync(model.Server, model.Driver)));

        [HttpPost]
        public async Task<IActionResult> GetTables([FromBody] ConnectionViewModel model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule29.GetTablesAsync(model.Server, model.Database, model.Driver)));

        [HttpPost]
        public async Task<IActionResult> GetColumns([FromBody] Rule29GetColumnsRequest model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule29.GetColumnsAsync(model.Server, model.Database, model.Driver, model.TableName)));

        [HttpPost]
        public async Task<IActionResult> LoadFilterValues([FromBody] Rule29FilterValueRequest model) =>
            Json(await RequireDataAnalystAsync(async () =>
                await _rule29.GetFilterValuesAsync(model.Server, model.Database, model.Driver, model.TableName, model.FilterColumn)));

        [HttpPost]
        public async Task<IActionResult> VerifyTable([FromBody] Rule29VerifyRequest request) =>
            Json(await RequireDataAnalystAsync(async () => await _rule29.VerifyTableAsync(request)));

        [HttpPost]
        public async Task<IActionResult> RunValidation([FromBody] Rule29ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                return Json(new Rule29ValidationSummary
                {
                    Success = false,
                ...
                !string.Equals(engagementRole, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    Success = false,
                    Error = "Only the assigned data analyst can run Rule 29."
                });
            }

            async Task<Rule29ValidationSummary> ExecuteValidationAsync(IRule29Service ruleService, IAuditLogService auditService)
            {
                ...
                if (result.Success)
                {
                    await auditService.LogAsync(
                        "run_validation",
                        $"Rule 29 on client {request.ClientId}: {result.Status} ({result.MatchingCount} matches), run {result.SavedRunId}",
                        user?.Id,
                        user?.Email);
                }
            ...
            if (ValidationOperationHttpHelper.IsAsyncRequested(Request))
            {
                return ValidationOperationHttpHelper.Queue(
                    ...
                    async (sp, ct) => await ExecuteValidationAsync(
                        sp.GetRequiredService<IRule29Service>(),
                        sp.GetRequiredService<IAuditLogService>()));
            }

            var result = await ExecuteValidationAsync(_rule29, _audit);
            return Json(result);
        }

        [HttpPost]
        public async Task<IActionResult> BeginWorkspaceEdit([FromBody] Rule29ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                return Json(new Rule29WorkspaceSaveResult
                {
                    Success = false,
            ...
            var result = await _rule29.BeginWorkspaceEditAsync(request.RunId.Value, user!.Email!, user.FullName);
            if (result.Success)
            {
                await _audit.LogAsync(
                    "workspace_edit_started",
                    $"DataAnalyst started editing Rule 29 run {request.RunId.Value}. Existing signoffs were cleared.",
                    user?.Id,
                    user?.Email);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> SaveWorkspace([FromBody] Rule29ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            var result = await _rule29.SaveWorkspaceAsync(request, user!.Email!, user.FullName);
            if (result.Success)
            {
                await _audit.LogAsync(
                    "save_validation_workspace",
                    $"DataAnalyst saved Rule 29 workspace for client {request.ClientId}. Signoffs cleared: {result.ClearedSignoffCount ?? 0}",
                    user?.Id,
                    user?.Email);
            }
        ...
        [HttpPost]
        public async Task<IActionResult> SignOffWorkspace([FromBody] Rule29WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            var review = await _rule29.GetSavedRunAsync(model.RunId.Value, user?.Email);
            ...
            try
            {
                await _rule29.AddOrUpdateSignoffAsync(model.RunId.Value, user!.Email!, model.Comment);
                await _audit.LogAsync(
                    "signoff_validation_run",
                    $"Rule 29 signoff saved for run {model.RunId.Value} from module workspace",
                    user.Id,
                    user.Email);
            }
            catch (Exception ex)
            ...
            var workspace = await _rule29.GetCurrentWorkspaceStateAsync(model.ClientId, user?.Email, includeSummary: false);
            var resultsVisible = CanViewWorkspaceResults(role, workspace);
        ...
        [HttpPost]
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule29WorkspaceSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            try
            {
                await _rule29.RemoveSignoffAsync(model.RunId.Value, user!.Email!);
            }
            catch (Exception ex)
            {
                return Json(new { success = false, error = ex.Message });
            ...
            await _audit.LogAsync(
                ...
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 29 run {model.RunId.Value} from module workspace. Historical snapshot preserved; new current run {reopenedRunId.Value} created for continued review."
                    : $"{review.CurrentUserEngagementRole} removed signoff for Rule 29 run {model.RunId.Value} from module workspace",
                user.Id,
                user.Email);
        ...
        [HttpPost]
        public async Task<IActionResult> GenerateSql([FromBody] Rule29ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            ...
            {
                return Json(new Rule29SqlResult
                {
                    Success = false,
                    Error = "You cannot access this engagement."
                });
            ...
            return Json(await RequireDataAnalystAsync(async () =>
                ...
                {
                    Success = true,
                    Sql = await _rule29.GenerateSqlAsync(request)
                }));
        }
        [HttpPost]
        public async Task<IActionResult> GenerateRScript([FromBody] Rule29ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);

            if (request.ClientId > 0 && !await _systemDb.CanAccessClientResultsAsync(request.ClientId, user, role))
                return Json(new Rule29SqlResult { Success = false, Error = "You cannot access this engagement." });

            return Json(await RequireDataAnalystAsync(async () => new Rule29SqlResult
            {
                Success = true,
                Sql = Rule29RScriptGenerator.Generate(request) + RScriptScaffold.BuildAutoExportFooter("Rule29")
            }));
        }

        [HttpPost, ValidateAntiForgeryToken]
        public async Task<IActionResult> AddSignoff(Rule29RunSignoffInputModel model)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule29.GetSavedRunAsync(model.RunId, user?.Email);
            if (review == null)
                return NotFound();
            ...
            await _rule29.AddOrUpdateSignoffAsync(model.RunId, user!.Email!, model.Comment);
            await _audit.LogAsync(
                "signoff_validation_run",
                $"{review.CurrentUserEngagementRole} signed off Rule 29 run {model.RunId}",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule29.GetSavedRunAsync(runId, user?.Email);
            if (review == null)
                return NotFound();
            ...
            await _rule29.RemoveSignoffAsync(runId, user!.Email!);
            var workspace = await _rule29.GetCurrentWorkspaceStateAsync(review.ClientId, user?.Email, includeSummary: false);
            var redirectRunId = workspace?.RunId ?? runId;
            var preservedHistory = workspace?.RunId.HasValue == true && workspace.RunId.Value != runId;
            await _audit.LogAsync(
                "remove_validation_signoff",
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 29 run {runId}. Historical snapshot preserved; new current run {redirectRunId} created for continued review."
                    : $"{review.CurrentUserEngagementRole} removed signoff for Rule 29 run {runId}",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> DownloadSavedSql(int runId)
        {
            ...
            var request = new Rule29ValidationRequest
            {
                ClientId = review.ClientId,
                Database = review.Summary.Database,
                TableName = review.Summary.TableName,
            ...
            var bytes = _export.ExportSql(await _rule29.GenerateSqlAsync(request));
            return File(bytes, "application/sql", $"Rule29_Single_Column_Filter_{runId}.sql");
        }
        ...
        [HttpPost]
        public async Task<IActionResult> DownloadSql([FromBody] Rule29ValidationRequest request)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            ...
            var fileName = $"Rule29_Single_Column_Filter_{Ts()}.sql";
            var bytes = _export.ExportSql(await _rule29.GenerateSqlAsync(request));
            return File(bytes, "application/sql", fileName);
        }

        private async Task<Rule29RunReviewViewModel?> LoadAuthorizedSavedRunAsync(int runId, bool requireDownloadAccess)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule29.GetSavedRunAsync(runId, user?.Email);
        ...
        private static bool CanDownloadSavedRun(Rule29RunReviewViewModel review, string systemRole)
        ...
        private static bool CanViewSavedRun(Rule29RunReviewViewModel review, string systemRole)
        ...
        private static bool CanViewWorkspaceResults(string role, Rule29WorkspaceStateViewModel? workspace)
        {
            if (workspace == null)
                return false;
        ...
        private async Task<Rule29ValidationSummary> ResolveExportSummaryAsync(Rule29ValidationSummary summary)
        {
            ...
            {
                var review = await _rule29.GetSavedRunAsync(savedRunId, user?.Email);
            ...
            {
                var workspace = await _rule29.GetCurrentWorkspaceStateAsync(summary.ClientId, user?.Email, includeSummary: false);
                if (workspace?.RunId is int workspaceRunId && workspaceRunId > 0)
                {
                    var review = await _rule29.GetSavedRunAsync(workspaceRunId, user?.Email);
                    if (review?.Summary != null)
                        return review.Summary;
                }
            }
        ...
        private async Task<object> RequireDataAnalystAsync<T>(Func<Task<T>> action)
        {
            ...
            if (!string.Equals(role, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            {
                return new { success = false, error = "Only the assigned data analyst can configure or run Rule 29." };
            }
