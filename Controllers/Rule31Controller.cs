    ...
    public class Rule31Controller : Controller
    {
        ...
        public async Task<IActionResult> Index(int clientId = 0)
        {
            ...
            if (IsResultsOnlyRole(role))
            {
                ...
                TempData["Error"] = "No analyst-signed Rule 31 result is available for this engagement yet.";
        ...
        public async Task<IActionResult> GetDatabases([FromBody] ConnectionViewModel model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule31.GetDatabasesAsync(model.Server, model.Driver)));

        [HttpPost]
        public async Task<IActionResult> GetTables([FromBody] ConnectionViewModel model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule31.GetTablesAsync(model.Server, model.Database, model.Driver)));

        [HttpPost]
        public async Task<IActionResult> GetColumns([FromBody] Rule31GetColumnsRequest model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule31.GetColumnsAsync(model.Server, model.Database, model.Driver, model.TableName)));
        ...
        public async Task<IActionResult> LoadFilterValues([FromBody] Rule31FilterValueRequest model) =>
            Json(await RequireDataAnalystAsync(async () =>
                await _rule31.GetFilterValuesAsync(model.Server, model.Database, model.Driver, model.TableName, model.ErrorTypeColumn)));
        ...
        public async Task<IActionResult> RunValidation([FromBody] Rule31ValidationRequest request)
        {
            ...
            var result = await _rule31.RunValidationAsync(request, user?.Email, user?.FullName ?? user?.Email);
            if (result.Success)
            {
        ...
        public async Task<IActionResult> SignOffWorkspace([FromBody] Rule31WorkspaceSignoffInputModel model)
        {
            ...
            var workspace = await _rule31.GetCurrentWorkspaceStateAsync(model.ClientId, user?.Email, includeSummary: false);
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            ...
            return File(bytes, "text/csv", $"Rule31_Excluded_and_Remaining_Run_{runId}.csv");
        }
        ...
        public async Task<IActionResult> DownloadSavedExceptionsCsv(int runId)
        {
            ...
            return File(bytes, "text/csv", $"Rule31_Remaining_Errors_Run_{runId}.csv");
        }
        ...
        [HttpPost]
        public IActionResult DownloadExcel([FromBody] Rule31ValidationSummary summary)
        {
        ...
        [HttpPost]
        public IActionResult DownloadCsv([FromBody] Rule31ValidationSummary summary)
        {
        ...
        [HttpPost]
        public IActionResult DownloadExceptionsCsv([FromBody] Rule31ValidationSummary summary)
        {
