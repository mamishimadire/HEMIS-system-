    ...
    public class Rule11Controller : Controller
    {
        ...
        public async Task<IActionResult> GetWorkspaceState(int clientId)
        {
            ...
            if (workspace != null && !resultsVisible)
            {
                ...
                workspace.ExcludeMPrefixPattern = true;
                workspace.PostgraduateTypesCsv = "07,27,28,49,72,73,08,30,50,74,75";
        ...
        public async Task<IActionResult> Run(int id)
        {
            ...
            review.GeneratedSql = await _rule11.GenerateSqlAsync(new Rule11ValidationRequest
            {
                ClientId = review.ClientId,
                Server = review.SourceServer,
                Database = review.Summary.Database,
                QualTable = review.Summary.QualTable,
                ExcludeMPrefixPattern = review.Summary.ExcludeMPrefixPattern,
        ...
        public async Task<IActionResult> VerifyTables([FromBody] Rule11VerifyRequest request) =>
            Json(await RequireDataAnalystAsync(async () => await _rule11.VerifyTablesAsync(request)));
        ...
        public async Task<IActionResult> RunValidation([FromBody] Rule11ValidationRequest request)
        {
            ...
            {
                return Json(new Rule11ValidationSummary
                {
                    Success = false,
                ...
                !string.Equals(engagementRole, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    Success = false,
                    Error = "Only the assigned data analyst can run Rule 11."
                });
            }

            async Task<Rule11ValidationSummary> ExecuteValidationAsync(IRule11Service ruleService, IAuditLogService auditService)
            {
                ...
                if (result.Success)
                {
                    await auditService.LogAsync(
                        "run_validation",
                        $"Rule 11 on client {request.ClientId}: {result.Status} ({result.FailCount} fail rows). Validation completed but is not saved until Save Workspace is clicked.",
                        user?.Id,
                        user?.Email);
                }
            ...
            var result = await ExecuteValidationAsync(_rule11, _audit);
            return Json(result);
        }
        ...
        public async Task<IActionResult> BeginWorkspaceEdit([FromBody] Rule11ValidationRequest request)
        {
            ...
            {
                return Json(new Rule11WorkspaceSaveResult
                {
                    Success = false,
            ...
            if (result.Success)
            {
                await _audit.LogAsync(
                    ...
                    $"DataAnalyst started editing Rule 11 run {request.RunId.Value}. A new validation must be saved before signoff.",
                    user?.Id,
                    user?.Email);
            }
        ...
        public async Task<IActionResult> SaveWorkspace([FromBody] Rule11ValidationRequest request)
        {
            ...
            if (result.Success)
            {
                await _audit.LogAsync(
                    ...
                    $"DataAnalyst saved Rule 11 workspace for client {request.ClientId}. Current run: {result.Workspace?.RunId}",
                    user?.Id,
                    user?.Email);
            }
        ...
        public async Task<IActionResult> SignOffWorkspace([FromBody] Rule11WorkspaceSignoffInputModel model)
        {
            ...
            try
            {
                ...
                await _audit.LogAsync(
                    ...
                    $"Rule 11 signoff saved for run {model.RunId.Value} from module workspace",
                    user.Id,
                    user.Email);
            }
            catch (Exception ex)
            ...
            var workspace = await _rule11.GetCurrentWorkspaceStateAsync(model.ClientId, user?.Email, includeSummary: false);
            var resultsVisible = CanViewWorkspaceResults(role, workspace);
        ...
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule11WorkspaceSignoffInputModel model)
        {
            ...
            await _audit.LogAsync(
                ...
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 11 run {model.RunId.Value} from module workspace. Historical snapshot preserved; new current run {reopenedRunLabel} created for continued review."
        ...
        public async Task<IActionResult> GenerateSql([FromBody] Rule11ValidationRequest request)
        {
            ...
            {
                return Json(new Rule11SqlResult
                {
                    Success = false,
                    Error = "You cannot access this engagement."
                });
            ...
            return Json(await RequireDataAnalystAsync(async () =>
                ...
                {
                    Success = true,
                    Sql = await _rule11.GenerateSqlAsync(request)
                }));
        }
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            ...
            var workspace = await _rule11.GetCurrentWorkspaceStateAsync(review.ClientId, user?.Email, includeSummary: false);
            var redirectRunId = workspace?.RunId ?? runId;
            var preservedHistory = workspace?.RunId.HasValue == true && workspace.RunId.Value != runId;
            await _audit.LogAsync(
                "remove_validation_signoff",
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 11 run {runId}. Historical snapshot preserved; new current run {redirectRunId} created for continued review."
        ...
        public async Task<IActionResult> DownloadSavedExcel(int runId)
        {
            ...
            return File(bytes, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", $"Rule11_Qualification_Selection_Run_{runId}.xlsx");
        }
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            ...
            return File(bytes, "text/csv", $"Rule11_Qualification_Selection_Run_{runId}.csv");
        }
        ...
        public async Task<IActionResult> DownloadSavedSql(int runId)
        {
            ...
            var bytes = _export.ExportSql(await _rule11.GenerateSqlAsync(request));
            return File(bytes, "application/sql", $"Rule11_Qualification_Selection_{runId}.sql");
        }
        ...
        public async Task<IActionResult> DownloadExcel([FromBody] Rule11ValidationSummary summary)
        {
            ...
            return File(bytes, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", $"Rule11_Qualification_Selection_{Ts()}.xlsx");
        }
        ...
        public async Task<IActionResult> DownloadCsv([FromBody] Rule11ValidationSummary summary)
        {
            ...
            return File(bytes, "text/csv", $"Rule11_Qualification_Selection_{Ts()}.csv");
        }
        ...
        public async Task<IActionResult> DownloadSql([FromBody] Rule11ValidationRequest request)
        {
            ...
            var bytes = _export.ExportSql(await _rule11.GenerateSqlAsync(request));
            return File(bytes, "application/sql", $"Rule11_Qualification_Selection_{Ts()}.sql");
        }
        ...
        private async Task<Rule11RunReviewViewModel?> LoadAuthorizedSavedRunAsync(int runId, bool requireDownloadAccess)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule11.GetSavedRunAsync(runId, user?.Email, includeFullResults: requireDownloadAccess);
            if (review == null)
            {
        ...
        private async Task<Rule11ValidationSummary> ResolveExportSummaryAsync(Rule11ValidationSummary summary)
        {
            ...
            {
                var review = await _rule11.GetSavedRunAsync(savedRunId, user?.Email, includeFullResults: true);
            ...
            {
                var workspace = await _rule11.GetCurrentWorkspaceStateAsync(summary.ClientId, user?.Email, includeSummary: false);
                if (workspace?.RunId is int workspaceRunId && workspaceRunId > 0)
                {
                    var review = await _rule11.GetSavedRunAsync(workspaceRunId, user?.Email, includeFullResults: true);
                    if (review?.Summary != null)
                        return review.Summary;
                }
            }
        ...
        private async Task<object> RequireDataAnalystAsync<T>(Func<Task<T>> action) where T : class
        {
            ...
            if (result == null)
                return new { success = false, error = "Rule 11 action returned no result." };
