    ...
    public class Rule27Controller : Controller
    {
        ...
        public async Task<IActionResult> Index(int clientId = 0)
        {
            ...
            if (IsResultsOnlyRole(role))
            {
                ...
                TempData["Error"] = "No analyst-signed Rule 27 result is available for this engagement yet.";
        ...
        public async Task<IActionResult> RunValidation([FromBody] Rule27ValidationRequest request)
        {
            ...
            var result = await _rule27.RunValidationAsync(request, user?.Email, user?.FullName ?? user?.Email);
            if (result.Success)
            {
        ...
        [HttpPost]
        public IActionResult DownloadExcel([FromBody] Rule27ValidationSummary summary)
        {
        ...
        [HttpPost]
        public IActionResult DownloadCsv([FromBody] Rule27ValidationSummary summary)
        {
