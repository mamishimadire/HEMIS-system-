...
namespace HemisAudit.Controllers
{
    ...
    public class Rule34Controller : Controller
    {
        ...
        public async Task<IActionResult> RunValidation([FromBody] Rule34ValidationRequest request)
        {
            ...
            async Task<Rule34ValidationSummary> ExecuteValidationAsync(IRule34Service ruleService, IAuditLogService auditService)
            {
            ...
            if (ValidationOperationHttpHelper.IsAsyncRequested(Request))
            {
                return ValidationOperationHttpHelper.Queue(
                    ...
                    async (sp, ct) => await ExecuteValidationAsync(
                        sp.GetRequiredService<IRule34Service>(),
                        sp.GetRequiredService<IAuditLogService>()));
            }

            var result = await ExecuteValidationAsync(_rule34, _audit);
            return Json(result);
        }
        ...
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule34WorkspaceSignoffInputModel model)
        {
            ...
            await _audit.LogAsync(
                ...
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 34 run {model.RunId.Value} from module workspace. Historical snapshot preserved; new current run {reopenedRunId.Value} created for continued review."
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            ...
            var workspace = await _rule34.GetCurrentWorkspaceStateAsync(review.ClientId, user?.Email, includeSummary: false);
            var redirectRunId = workspace?.RunId ?? runId;
            var preservedHistory = workspace?.RunId.HasValue == true && workspace.RunId.Value != runId;
            await _audit.LogAsync(
                "remove_validation_signoff",
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 34 run {runId}. Historical snapshot preserved; new current run {redirectRunId} created for continued review."
        ...
        [HttpPost]
        public async Task<IActionResult> DownloadExcel([FromBody] Rule34ValidationSummary summary)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> DownloadCsv([FromBody] Rule34ValidationSummary summary)
        {
            summary = await ResolveExportSummaryAsync(summary);
        ...
        [HttpPost]
        public async Task<IActionResult> DownloadExceptionsCsv([FromBody] Rule34ValidationSummary summary)
        {
            summary = await ResolveExportSummaryAsync(summary);
        ...
        private async Task<Rule34ValidationSummary> ResolveExportSummaryAsync(Rule34ValidationSummary summary)
        {
            var user = await _users.GetUserAsync(User);

            if (summary.SavedRunId is int savedRunId && savedRunId > 0)
            {
                var review = await _rule34.GetSavedRunAsync(savedRunId, user?.Email);
            ...
            {
                var workspace = await _rule34.GetCurrentWorkspaceStateAsync(summary.ClientId, user?.Email, includeSummary: false);
                if (workspace?.RunId is int workspaceRunId && workspaceRunId > 0)
                {
                    var review = await _rule34.GetSavedRunAsync(workspaceRunId, user?.Email);
                    if (review?.Summary != null)
                        return review.Summary;
                }
            }
