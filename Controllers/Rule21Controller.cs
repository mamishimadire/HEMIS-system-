...
namespace HemisAudit.Controllers
{
    ...
    public class Rule21Controller : Controller
    {
        ...
        public async Task<IActionResult> GetColumns([FromBody] Rule21GetColumnsRequest model) =>
            Json(await RequireDataAnalystAsync(async () => await _rule21.GetColumnsAsync(model.Server, model.Database, model.Driver, model.TableName)));

        [HttpPost]
        public async Task<IActionResult> LoadFilterValues([FromBody] Rule21FilterValueRequest model) =>
            Json(await RequireDataAnalystAsync(async () =>
                await _rule21.GetFilterValuesAsync(model.Server, model.Database, model.Driver, model.TableName, model.FilterColumn)));

        [HttpPost]
        public async Task<IActionResult> VerifyTable([FromBody] Rule21VerifyRequest request) =>
            Json(await RequireDataAnalystAsync(async () => await _rule21.VerifyTableAsync(request)));
        ...
        public async Task<IActionResult> RunValidation([FromBody] Rule21ValidationRequest request)
        {
            ...
            {
                return Json(new Rule21ValidationSummary
                {
                    Success = false,
                ...
                !string.Equals(engagementRole, "DataAnalyst", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    Success = false,
                    Error = "Only the assigned data analyst can run Rule 21."
                });
            }

            async Task<Rule21ValidationSummary> ExecuteValidationAsync(IRule21Service ruleService, IAuditLogService auditService)
            {
                ...
                if (result.Success)
                {
                    await auditService.LogAsync(
                        "run_validation",
                        $"Rule 21 on client {request.ClientId}: {result.Status} ({result.MatchingCount} matches), run {result.SavedRunId}",
                        user?.Id,
                        user?.Email);
                }
            ...
            var result = await ExecuteValidationAsync(_rule21, _audit);
            return Json(result);
        }
        ...
        public async Task<IActionResult> BeginWorkspaceEdit([FromBody] Rule21ValidationRequest request)
        {
            ...
            {
                return Json(new Rule21WorkspaceSaveResult
                {
                    Success = false,
        ...
        public async Task<IActionResult> SignOffWorkspace([FromBody] Rule21WorkspaceSignoffInputModel model)
        {
            ...
            try
            {
                ...
                await _audit.LogAsync(
                    ...
                    $"Rule 21 signoff saved for run {model.RunId.Value} from module workspace",
                    user.Id,
                    user.Email);
            }
            catch (Exception ex)
            ...
            var workspace = await _rule21.GetCurrentWorkspaceStateAsync(model.ClientId, user?.Email, includeSummary: false);
            var resultsVisible = CanViewWorkspaceResults(role, workspace);
        ...
        public async Task<IActionResult> RemoveWorkspaceSignoff([FromBody] Rule21WorkspaceSignoffInputModel model)
        {
            ...
            try
            {
                await _rule21.RemoveSignoffAsync(model.RunId.Value, user!.Email!);
            }
            catch (Exception ex)
            {
                return Json(new { success = false, error = ex.Message });
            ...
            await _audit.LogAsync(
                ...
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 21 run {model.RunId.Value} from module workspace. Historical snapshot preserved; new current run {reopenedRunId.Value} created for continued review."
                    : $"{review.CurrentUserEngagementRole} removed signoff for Rule 21 run {model.RunId.Value} from module workspace",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> GenerateSql([FromBody] Rule21ValidationRequest request)
        {
            ...
            {
                return Json(new Rule21SqlResult
                {
                    Success = false,
                    Error = "You cannot access this engagement."
                });
        ...
        public async Task<IActionResult> AddSignoff(Rule21RunSignoffInputModel model)
        {
            ...
            await _audit.LogAsync(
                "signoff_validation_run",
                $"{review.CurrentUserEngagementRole} signed off Rule 21 run {model.RunId}",
                user.Id,
                user.Email);
        ...
        public async Task<IActionResult> RemoveSignoff(int runId)
        {
            ...
            var workspace = await _rule21.GetCurrentWorkspaceStateAsync(review.ClientId, user?.Email, includeSummary: false);
            var redirectRunId = workspace?.RunId ?? runId;
            var preservedHistory = workspace?.RunId.HasValue == true && workspace.RunId.Value != runId;
            await _audit.LogAsync(
                "remove_validation_signoff",
                preservedHistory
                    ? $"{review.CurrentUserEngagementRole} removed signoff for Rule 21 run {runId}. Historical snapshot preserved; new current run {redirectRunId} created for continued review."
        ...
        public async Task<IActionResult> DownloadSavedExcel(int runId)
        {
            ...
            return File(bytes,
                ...
                $"Rule21_Filtered_Results_Run_{runId}.xlsx");
        }
        ...
        public async Task<IActionResult> DownloadSavedCsv(int runId)
        {
            ...
            return File(bytes, "text/csv", $"Rule21_Filtered_Results_Run_{runId}.csv");
        }
        ...
        public async Task<IActionResult> DownloadSavedSql(int runId)
        {
            ...
            return File(bytes, "application/sql", $"Rule21_First_Time_Entering_{runId}.sql");
        }
        ...
        public async Task<IActionResult> DownloadExcel([FromBody] Rule21ValidationSummary summary)
        {
            ...
            var fileName = $"Rule21_Filtered_Results_{Ts()}.xlsx";
            return File(bytes,
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                fileName);
        }
        ...
        {
            summary = await ResolveExportSummaryAsync(summary);
            var fileName = $"Rule21_Filtered_Results_{Ts()}.csv";
        ...
        public async Task<IActionResult> DownloadSql([FromBody] Rule21ValidationRequest request)
        {
            ...
            var fileName = $"Rule21_First_Time_Entering_{Ts()}.sql";
            var bytes = _export.ExportSql(await _rule21.GenerateSqlAsync(request));
        ...
        private async Task<Rule21RunReviewViewModel?> LoadAuthorizedSavedRunAsync(int runId, bool requireDownloadAccess)
        {
            var user = await _users.GetUserAsync(User);
            var role = await GetCurrentSystemRoleAsync(user);
            var review = await _rule21.GetSavedRunAsync(runId, user?.Email, includeFullResults: requireDownloadAccess);
            if (review == null)
            {
        ...
        private async Task<Rule21ValidationSummary> ResolveExportSummaryAsync(Rule21ValidationSummary summary)
        {
            ...
            {
                var review = await _rule21.GetSavedRunAsync(savedRunId, user?.Email, includeFullResults: true);
            ...
            {
                var workspace = await _rule21.GetCurrentWorkspaceStateAsync(summary.ClientId, user?.Email, includeSummary: false);
                if (workspace?.RunId is int workspaceRunId && workspaceRunId > 0)
                {
                    var review = await _rule21.GetSavedRunAsync(workspaceRunId, user?.Email, includeFullResults: true);
                    if (review?.Summary != null)
                        return review.Summary;
                }
            }
